...
?>
	...
	<option 
	<?php	if (!empty($login_info)) {  echo $login_info->role_id == 4 ? 'selected' : '';
                                        }
                                        ?> value="4">Employee</option> 
                            </select>
                        </div>
                    </div>
                    ...
                    ?>
					 <div class="form-group">
                        <label for="direction" class="control-label col-sm-3"><strong>User Login</strong><span class="required">*</span></label>
                        <div class="col-sm-5">
                            <select name="user_login" id="user_login" onchange = "userloginShowHideDiv()" class="form-control" required>
							<option> Select</option>
                                <option  value="HRM" <?php if (!empty($login_info)) { if($login_info->user_login =="HRM") { echo "selected"; }} ?>>HRM</option>
                                <option value="CRM" <?php if (!empty($login_info)) { if($login_info->user_login=="CRM") { echo "selected"; }} ?>>CRM</option>
                            </select>
                        </div>
                    </div>
                   <!-- <div class="form-group">
                        <label for="direction" class="control-label col-sm-3"></label>
                        <div class="col-sm-5">
                    ...
                    <div class="col-sm-12" <?php if (!empty($login_info) && $login_info->role_id == 2) { echo 'style="display:block"'; } ?>>
                         ...
                         <label class="col-sm-3 control-label"><strong><?= lang('companies') ?></strong></label>
                          <div class="col-sm-6">
                           <select class="form-control select_box" style="width: 100%" name="company">
                              ...
                              <?php if (!empty($all_client_info)) {
                              foreach ($all_client_info as $v_client) { ?>
                              <option value="<?= $v_client->client_id ?>" <?php if (!empty($profile_info)) { if ($profile_info->company == $v_client->client_id) { echo 'selected'; } } ?>> <?= $v_client->name ?></option> <?php } } ?>
                           </select>
                          </div>
                      </div>
                        <?php
                                ...
                                <div class="col-lg-6 checkbox">
                                    ...
                                    if (!empty($user_menu)) {
                                        foreach ($user_menu as $v_u_menu) {
                                            if ($v_u_menu->menu_id == $v_menu->menu_id) { echo 'checked'; } } 
                                    } else {
                                        $client_menu = unserialize(config_item('client_default_menu'));
                                        if (!empty($client_menu['client_default_menu'])) {
                                            foreach ($client_menu['client_default_menu'] as $default_menu) { echo $v_menu->menu_id == $default_menu ? 'checked' : ''; } } 
				...
				<div class="col-sm-12" <?php if (!empty($login_info) && $login_info->role_id == 2) { echo 'style="display:block"'; } ?>>
                        <div class="col-sm-3"></div>
                        <div class="col-sm-6 row">
                            <div id="employee_permission" class="panel panel-custom ">
                                <div class="panel-heading">
                                    <h4 class="modal-title" id="myModalLabel">
                                       Select Employee permission</h4>
                                </div>
                                <style type="text/css">
                                    .toggle.btn-xs {
                                        min-width: 59px;
                                  ...
                                  <div class="form-group">
                                     ...
                                     <label class="col-sm-3 control-label"><strong><?= lang('companies') ?></strong></label>
                                      <div class="col-sm-6">
                                       ...
                                       <select class="form-control select_box" style="width: 100%" name="company">
                                          <option value="-"><?= lang('select_client') ?></option>
                                          <?php if (!empty($all_client_info)) { foreach ($all_client_info as $v_client) { ?>
                                           <option value="<?= $v_client->client_id ?>" <?php if (!empty($profile_info)) { if ($profile_info->company == $v_client->client_id) { echo 'selected'; } } ?>><?= $v_client->name ?></option> <?php } } ?>
                                       </select>
                                      </div>
                                  </div>
                                    ...
                                    ?>
                                        <div class="form-group">
                                            <label class="col-lg-3 control-label"><?= lang($v_menu->label) ?></label>
                                            <div class="col-lg-6 checkbox">
                                                <input class="employee_permission" data-toggle="toggle" name="<?= $v_menu->label ?>" value="<?= $v_menu->menu_id ?>" <?php if (!empty($user_menu)) { foreach ($user_menu as $v_u_menu) { if ($v_u_menu->menu_id == $v_menu->menu_id) { echo 'checked'; } } } else { $client_menu = unserialize(config_item('client_default_menu')); if (!empty($client_menu['client_default_menu'])) { foreach ($client_menu['client_default_menu'] as $default_menu) { echo $v_menu->menu_id == $default_menu ? 'checked' : ''; } } } ?> data-on="<?= lang('yes') ?>" data-off="<?= lang('no') ?>" data-onstyle="success btn-xs" data-offstyle="danger btn-xs" type="checkbox">
                                            </div>
                                        </div>
                                    <?php
                                    }
                ...
                <div class="col-sm-12" <?php if (!empty($login_info) && $login_info->role_id == 3) { echo 'style="display:block"'; } ?>>
                        <div class="col-sm-3"></div>
                        <div class="col-sm-6 row">
                            <div id="staff_permission" class="panel panel-custom ">
                                <div class="panel-heading">
                                    <h4 class="modal-title" id="myModalLabel">
                                       Select Staff permission</h4>
                                </div>
                                <style type="text/css">
                                    .toggle.btn-xs {
                                        min-width: 59px;
                                ...
                                <div class="form-horizontal">
                                    <?php
                                    $all_client_menu = $this->db->where('parent', 0)->order_by('sort')->get('tbl_menu')->result();
                                    if (!empty($login_info)) {
                                        $user_menu = $this->db->where('user_id', $login_info->user_id)->get('tbl_client_role')->result();
                                    }
                                    ...
                                    ?>
                                        <div class="form-group">
                                            <label class="col-lg-3 control-label"><?= lang($v_menu->label) ?></label>
                                            <div class="col-lg-6 checkbox">
                                                <input class="staff_permission" data-toggle="toggle" name="<?= $v_menu->label ?>" value="<?= $v_menu->menu_id ?>" <?php 
                                                if (!empty($user_menu)) { 
                                                    foreach ($user_menu as $v_u_menu) {
                                                        if ($v_u_menu->menu_id == $v_menu->menu_id) { echo 'checked'; } 
                                                    }
				...
				<!---------employee-->
                    ...
                    <div class="form-group" id="department" <?php if (!empty($login_info) && $login_info->role_id != 2) { echo 'style="display:block"'; } ?>>
                        ...
                        <div class="col-sm-5">
                            <div class="input-group">
                                <select class="form-control select_box department" style="width: 100%" name="designations_id">
                                    ...
                                    <?php if (!empty($all_designation_info)) {
                                    ...
                                    <?php if (!empty($v_designation_info)) {
                                        ...
                                        <option value="<?= $v_designation->designations_id ?>" <?php if (!empty($profile_info)) { if ($profile_info->designations_id == $v_designation->designations_id) { echo 'selected'; } } ?>> <?= $v_designation->designations; ?></option>
                                    <?php } } ?>
                                    </optgroup>
                                    <?php } } ?>
                                </select>
    ...
    <script>
        <?php if (!empty($edited)) { ?>
            $(document).ready(function() {
				...
				$('#employee_permission').hide();
                $('#staff_permission').hide();
                ...
                $('#user_type').on('change', function() {
                    if (this.value == '1') {
                        ...
                        $('#staff_permission').hide();
                        $(".staff_permission").attr('disabled', 'disabled');
                        $('#client_permission').hide();
                        $(".client_permission").attr('disabled', 'disabled');
						$('#employee_permission').hide();
                        $(".employee_permission").attr('disabled', 'disabled');
                      //  $(".department").attr('required', true);
                        ...
                        $('#client_permission').hide();
                        $(".client_permission").attr('disabled', 'disabled');
                        $('#staff_permission').show();
                        $(".staff_permission").removeAttr('disabled');
						$('#employee_permission').hide();
                        $(".employee_permission").attr('disabled', 'disabled');
                        $("#department").hide();
                        $(".department").attr('disabled', 'disabled');
                        ...
                        $('#employee_permission').show();
                        $(".employee_permission").removeAttr('disabled');
                        $('#staff_permission').hide();
                        $(".staff_permission").attr('disabled', 'disabled');
						$('#client_permission').hide();
                        $(".client_permission").attr('disabled', 'disabled');
    ...
    if (!empty($login_info) && $login_info->role_id != 2) { ?>
        ...
        <?php }
            if (!empty($login_info) && $login_info->role_id == 3) { ?>
        <script>
            $(document).ready(function() {
                $('#staff_permission').show();
                $(".staff_permission").removeAttr('disabled');
	...
	<?php
            if (!empty($login_info) && $login_info->role_id == 4) { ?>
        <script>
            $(document).ready(function() {
                $('#employee_permission').show();
                $(".employee_permission").removeAttr('disabled');
	...
	<script type="text/javascript">
    function userloginShowHideDiv() {
        var user_login = document.getElementById("user_login");
        var department = document.getElementById("department");
        department.style.display = user_login.value == "HRM" ? "block" : "none";
