...
class Menu
{
    ...
    public function dynamicMenu()
    {
        ...
        if ($user_type == 2 || $user_type == 4) { // query for employee user role
            ...
            $CI->db->join('tbl_menu', 'tbl_client_role.menu_id = tbl_menu.menu_id', 'left');
            $CI->db->where('tbl_client_role.user_id', $user_id);
            $CI->db->group_by('tbl_menu.menu_id');
            $CI->db->order_by('sort', 'time');
            $query_result = $CI->db->get();
            $user_menus = $query_result->result_array();
            foreach($user_menus as $user){
                $CI->db->where('parent', $user['menu_id']);
                $CI->db->where('status', 1);
                $CI->db->order_by('sort');
                $CI->db->from('tbl_menu');
                $query_sub_result = $CI->db->get();
                $user_sub_menu = $query_sub_result->result_array();
                if(!empty($user_sub_menu)){
                    foreach($user_sub_menu as $u){
                        array_push($user_menus, $u);
                    }
                }
            }
            $user_menu = json_decode(json_encode($user_menus));
