	...
	public function index()
{
    ...
    if ($this->input->post('user_name')) {
        $admin = $this->login_model->get_admin($this->input->post('user_name'), $this->input->post('password'));

        if ($admin && $admin->activated && !$admin->banned) {
            // Check if the user's role_id is 1, 3 or 4
            if ($admin->role_id == 1 || $admin->role_id == 4 || $admin->role_id == 3) {
                // Login successful, set user data in session and redirect to dashboard
                $user_info = $this->login_model->get_user_info($admin->user_id);
                ...
                // Login failed, role_id not allowed
