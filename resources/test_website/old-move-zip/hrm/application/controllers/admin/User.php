...
class User extends Admin_Controller
{
    ...
    {
		if($this->input->post('user_login')=='HRM') {
		
        $created = can_action('24', 'created');
        $edited = can_action('24', 'edited');
        if (!empty($created) || !empty($edited) && !empty($id)) {
            $login_data = $this->user_model->array_from_post(array('username', 'email', 'role_id', 'user_login'));
            $admin = admin();
            ...
            if (!empty($check_user) || !empty($check_email)) { // if input data already exist show error alert
                ...
                if (!empty($id)) {
                    ...
                    if($login_data['role_id'] == 3){
                        ...
                        $all_client_menu = $this->db->get('tbl_menu')->result(); 
                    } else {
	...
	if($this->input->post('user_login')=='CRM') {
	 ...
	 if (empty($this->input->post('user_id', true))) {
			...
			$password= password_hash($pass, PASSWORD_BCRYPT, array('cost' => 8));
			//$password=$this->hash($pass);
			$skype=$this->input->post('skype');	
			$phonenumber=$this->input->post('phone');	
			$curentdate = date('Y-m-d H:i:s');
			$data = array(
				'hrm_user_id' =>$last_insert_id, 
				'firstname'=>$fullname,
				'lastname'=>$fullname,
				'email'=>$username,
				'password'=>$password,
				...
				'datecreated'=>$curentdate,
				'role'=>1,
				'active'=>1
			);
	
			$DB2 = $this->load->database('default1', TRUE);
			$DB2->insert('tblstaff', $data);
			//redirect('admin/user/user_list');
		 }
	...
	if (!empty($this->input->post('user_id', true))) {
		...
		//print_r($data);exit; 
			$DB2 = $this->load->database('default1', TRUE);
			$DB2->where('hrm_user_id', $id);
			$DB2->update('tblstaff', $data);
	}
        redirect('admin/user/user_list'); //redirect page
