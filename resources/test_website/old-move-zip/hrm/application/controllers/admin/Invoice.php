    ...
    public function invoiceList($filterBy = null, $search_by = null)
    {
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($fetch_data as $_key => $v_invoices) {
                if (!empty($v_invoices)) {
                    if (!empty($v_invoices->system_currency)) {
                        $currency = $this->invoice_model->check_by(array('code' => $v_invoices->system_currency), 'tbl_currencies');
                    } else {
                    ...
                    $sub_array[] = display_money($this->invoice_model->calculate_to('invoice_due', $v_invoices->invoices_id), $currency->symbol);
                    $recurring = null;
