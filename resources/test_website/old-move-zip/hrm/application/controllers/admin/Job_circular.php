...
class Job_Circular extends Admin_Controller
{
    ...
    public function save_job_posted($id = NULL)
    {
        ...
        if (!empty($created) || !empty($edited) && !empty($id)) {
            ...
            if (!empty($id)) {
                ...
                $msg =  "Job post information successfully updated";
            } else {
                $activity = 'activity_added_job_posted';
                $msg = "Job post information successfully saved";
                $id = $return_id;
            }
    ...
    function change_status($status, $id)
    {
        ...
        if (!empty($edited)) {
            ...
            $type = "success";
            $message = 'Job post status is changed to ' . ' ' . $status . '.';
            set_message($type, $message);
        }
        redirect('admin/job_circular/jobs_posted');
    }
   ...
   public function jobs_applicationsList($id = null)
    {
        ...
        if ($this->input->is_ajax_request()) {
           $this->load->model('datatables');
            ...
            $this->datatables->select = array('tbl_job_appliactions.*, tbl_job_circular.job_title, tbl_jobs.total_no_experience, tbl_jobs.choice_of_work, tbl_locations.name AS pname,tbl_designations.designations');
			$this->datatables->join_table = array('tbl_job_circular','tbl_jobs', 'tbl_locations','tbl_designations');
			$this->datatables->join_where = array('tbl_job_circular.job_circular_id=tbl_job_appliactions.job_circular_id',
            'tbl_jobs.id=tbl_job_appliactions.job_id',
            'tbl_locations.id=tbl_jobs.choice_of_work',
			 'tbl_designations.designations_id=tbl_jobs.designations_id',
            );
			$this->datatables->column_order = array('tbl_job_circular.job_title', 'tbl_job_appliactions.name', 'tbl_job_appliactions.email', 'tbl_job_appliactions.mobile', 'tbl_jobs.total_no_experience', 'tbl_jobs.choice_of_work', 'tbl_job_appliactions.application_status', 'tbl_job_appliactions.apply_date', 'tbl_job_appliactions.interview_date', 'tbl_job_appliactions.resume','tbl_designations.designations');
			$this->datatables->column_search = array('tbl_job_circular.job_title', 'tbl_job_appliactions.name', 'tbl_job_appliactions.email', 'tbl_job_appliactions.mobile', 'tbl_jobs.total_no_experience', 'tbl_locations.name', 'tbl_job_appliactions.application_status', 'tbl_job_appliactions.apply_date', 'tbl_job_appliactions.interview_date', 'tbl_job_appliactions.resume', 'tbl_jobs.choice_of_work','tbl_designations.designations');
            ...
            foreach ($fetch_data as $_key => $v_job_application) {
				...
				$arr_choice_of_work = explode (",", $v_job_application->choice_of_work);
				foreach($arr_choice_of_work as $work) {
				$work_area=$this->db->select('name')->from('tbl_locations')->where('id', $work)->get()->result();
				$arr .=$work_area[0]->name.',';
				}
				...
				if($v_job_application->job_title!=NULL){
                $sub_array[] = $title;
				}
				else {
				$sub_array[] = '<a data-toggle="modal" data-target="#myModal_lg" class="text-info" href="#">' . $v_job_application->designations . '</a>(Forms)';	
				}
                $sub_array[] = '<span class="tags">' . $v_job_application->name . '</span>';
                $sub_array[] = $v_job_application->email;
                $sub_array[] = $v_job_application->mobile;
				$sub_array[] =$v_job_application->total_no_experience;
				$sub_array[] =rtrim($arr, ',');
                ...
                $sub_array[] = $status;
			 $action .= '<a href="' . base_url() . 'uploads/' . $v_job_application->resume.'" class="btn btn-purple btn-xs" download="'.$v_job_application->resume.'" data-toggle="tooltip"><span
               ...
               /* $action .= '<a href="' . base_url() . 'admin/job_circular/jobs_application_details/' . $v_job_application->job_appliactions_id . '" title="' . lang('view') . '" class="btn btn-primary btn-xs" data-toggle="modal" data-target="#myModal"><span
                            class="fa fa-list-alt"></span></a>' . ' '; */
			$action .= '<a  target="_blank" href="' . base_url() . 'admin/job_circular/jobs_application_details/' . $v_job_application->job_appliactions_id . '" title="' . lang('view') . '" class="btn btn-primary btn-xs"><span
                            class="fa fa-list-alt"></span></a>' . ' ';
                if (!empty($deleted)) {
    ...
    function jobs_application_details($id)
    {
		...
		$data['job'] = $this->db->get_where('tbl_jobs', array('id' => $id))->row();
	
        $data['subview'] = $this->load->view('admin/job_circular/circular_datail_edit', $data, true);
		$this->load->view('admin/_layout_main', $data);
        // get salary template deatails
