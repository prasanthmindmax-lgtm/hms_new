...
class Job extends Admin_Controller
{
    ...
    {
		$data['menu'] = $this->job_model->call_menu();

        $user_id = $id;
        $data['active'] = 1;
        $data['title'] = 'Job List'; 
        $data['subview'] = $this->load->view('admin/job/user_list', $data, true);
    ...
    public function create($action = NULL, $id = NULL)
    {
     ...
     //   $data['all_designation_info'] = $this->user_model->all_designation();
        ...
        $data['subview'] = $this->load->view('admin/job/create', $data, true);
        $this->load->view('admin/_layout_main', $data);
    }
...
//print_r($_FILES);exit;
	...
	/*	$config = array(
			'upload_path' => "./uploads/",
			'allowed_types' => "docx|pdf|PDF|DOCX",
			'max_size' => "1024000", // file size , here it is 1 MB(1024 Kb)
		);
		//$this->load->library('upload', $config);
		...
		if ($this->upload->do_upload('resume')) {
			...
			$data['resume'] = $this->upload->data('file_name1');
		}*/ 
		
	  //echo $data['resume'];exit;
	...
	//$data['languages_know']=$this->input->post('languages_know');
	
	$data['language']=implode(',',$this->input->post('language'));
	//$data['languages_know']=implode(',',$this->input->post('languages_know'));
/*   $json1 = json_encode();
   $json2 = json_encode($data['languages_know']); */
  ...
  $insertField=[];
	...
	$response=$this->job_model->saverecords($data);
        
        redirect('admin/job/user_list'); //redirect page
  ...
  public function edit($id)
    {
		...
		$data['job'] = $this->db->get_where('tbl_jobs', array('id' => $user_id))->row();
	
        $data['subview'] = $this->load->view('admin/job/edit', $data, true);
        $this->load->view('admin/_layout_main', $data);
    }
...
//	print_r(count($language));
		...
		$data['languages_know']=json_encode($insertField);
	//print_r($data['languages_know']);exit;
	...
	//print_r($data);exit;
		...
		$this->job_model->update_job_id($id,$data);
		redirect('admin/job/user_list'); //redirect page
	}
		...
		public function delete_row($id) {   
			...
			$this->job_model->did_delete_row($id);
			redirect('admin/job/user_list'); //redirect page
		}
