...
$(function () {
  ...
  if (table_leads.length) {
    ...
    $.each(LeadsServerParams, function (i, obj) { console.log(obj);
...
function _lead_init_data(data, id) {
  ...
  if(id && id.length >= 10){ 
      $leadModal.find('.modal-body form #lead_form input #phonenumber').val(id);
  }
...
// Leads create popup
function init_lead_mobile(mobile){ 
    requestURL = "leads/lead/?mobile="+mobile+"&edit=true";

    requestGetJSON(requestURL)
    .done(function (response) {
      _lead_init_data(response, mobile);
    })
    .fail(function (data) {
      alert_float("danger", data.responseText);
    });
