...
<aside id="menu" class="sidebar sidebar">
    <ul class="nav metis-menu" id="side-menu">
        ...
        <li <?php if (get_option('show_setup_menu_item_only_on_hover') == 1) {
            ...
            $userData = $this->db->select('inbound_call, exotel, knowlarity')->from(db_prefix() . 'staff')->where('staffid', get_staff_user_id())->get()->row(); 
            if($userData->inbound_call ==1){ ?>
            <li class="menu-item-inbound">
                ...
                <ul class="nav nav-second-level collapse" aria-expanded="false">
                    <?php if($userData->exotel ==1){ ?>
                    <li class="sub-menu-item-calendar">
                        <a href="<?=base_url('admin/inbound/exotel')?>">
                            <span class="sub-menu-text">Exotel</span>
                        </a>
                    </li>
                    <?php } if($userData->knowlarity == 1){ ?>
                    <li class="sub-menu-item-calendar">
                        <a href="<?=base_url('admin/inbound/knowlarity')?>">
                            <span class="sub-menu-text">Knowlarity</span>
                        </a>
                    </li>
                    <?php } ?>
                </ul>
