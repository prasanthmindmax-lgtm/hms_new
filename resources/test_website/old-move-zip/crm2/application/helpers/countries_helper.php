...
{
    return hooks()->apply_filters('all_locations', get_instance()->db->order_by('id', 'asc')->get(db_prefix().'leads_locations')->result_array());
}
/**
...
/**
 * Get location row from database based on passed location id
 * @param  mixed $id
 * @return object
 */
function get_location($id)
{
    ...
    $location = $CI->app_object_cache->get('db-leads_locations-' . $id);

    if (!$location) {
        $CI->db->where('id', $id);
        $location = $CI->db->get(db_prefix().'leads_locations')->row();
        $CI->app_object_cache->add('db-leads_locations-' . $id, $location);
    }
