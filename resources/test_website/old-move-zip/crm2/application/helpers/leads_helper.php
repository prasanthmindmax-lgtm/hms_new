...
function get_leads_summary()
{
    ...
    if (get_staff_user_id() != 1) {
      $statuses = $CI->leads_model->get_staff_status();
    } else {
    ...
    if (get_staff_user_id() != 1) {
       $newQuery = '';
       $userData = $CI->db->select('lead_source, lead_location')->from(db_prefix() . 'staff')->where('staffid', get_staff_user_id())->get()->result_array(); 
       ...
       if($lead_source){
          $newQuery .= ' AND (`source` IN ('.$newSource.'))';
       } if($lead_location){
          $newQuery .= ' AND (`preferred_location` IN ('.$lead_location.'))';
       }
    } else {
       $whereNoViewPermission = ' AND (addedfrom = ' . get_staff_user_id() . ' OR assigned=' . get_staff_user_id() . ' OR is_public = 1)';
