...
class Leads_model extends App_Model
{
    ...
    public function add($data)
    {
        ...
        if (isset($data['walk_in_date'])) {
            $data['walk_in_date'] = to_sql_date($data['walk_in_date'], true);
        } else {
            $data['walk_in_date'] = null;
        }
    ...
    public function update($data, $id)
    {
        ...
        $this->db->where('id', $id);
        $assigned = $this->db->get(db_prefix() . 'leads')->row()->assigned;
    ...
    public function get_locations($id = false)
    {
        if (is_numeric($id)) {
            ...
            return $this->db->get(db_prefix() . 'leads_locations')->row();
        }

        $this->db->order_by('id', 'asc');

        return $this->db->get(db_prefix() . 'leads_locations')->result_array();
    }
     ...
     * @return mixed object if id passed else array
     */
    public function get_campaigner($id = false)
    {
        if (is_numeric($id)) {
            $this->db->where('id', $id);

            return $this->db->get(db_prefix() . 'campaigner')->row();
        }
     ...
     * @return mixed      object if id passed else array
     */
    public function get_telecaller()
    {
	...
	public function get_attributes($id = '', $where = [])
    {
        $this->db->where($where);
        if (is_numeric($id)) {
            $this->db->where('id', $id);

            return $this->db->get(db_prefix() . 'leads_attributes')->row();
        }

        $statuses = $this->app_object_cache->get('leads-all-attributes');

        if (!$statuses) {
            $this->db->order_by('id', 'asc');

            $statuses = $this->db->get(db_prefix() . 'leads_attributes')->result_array();
            $this->app_object_cache->add('leads-all-attributes', $statuses);
        }

        return $statuses;
    }
    
    public function get_staff_status($id = '', $where = [])
    {
        $this->db->where($where);
        
        $userData = $this->db->select('lead_status')->from(db_prefix() . 'staff')->where('staffid', get_staff_user_id())->get()->result_array();
        $lead_status = explode(',', $userData[0]['lead_status']); 
        $this->db->where_in('id',$lead_status);
	 ...
	 public function add_attribute($data)
    {
        ...
        $this->db->insert(db_prefix() . 'leads_attributes', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
            log_activity('New Leads Attribute Added [StatusID: ' . $insert_id . ', Name: ' . $data['name'] . ']');
	...
	public function update_attributes($data, $id)
    {
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'leads_attributes', $data);
        if ($this->db->affected_rows() > 0) {
            log_activity('Leads Attribute Updated [StatusID: ' . $id . ', Name: ' . $data['name'] . ']');
	    ...
	    public function delete_attributes($id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->delete(db_prefix() . 'leads_attributes');
        if ($this->db->affected_rows() > 0) {
            if (get_option('leads_default_status') == $id) {
                update_option('leads_default_status', '');
            }
            log_activity('Leads Attribute Deleted [StatusID: ' . $id . ']');
