...
public static function alphaID($in, $to_num = false, $pad_up = false, $passKey = null)
{
  ...
  static function ResumeUserStatus()
  {
	...
	static public function getHomePageBanner($id)
	{
		$banner = \App\Models\Homepagebanner::find($id);
		return $banner;
	}
	static public function getAwards($id)
	{
		$awards = \App\Models\Awardsgallery::find($id);
		return $awards;
	}
	static public function getTreatments($id)
	{
		$treatments = \App\Models\Treatments::find($id);
		return $treatments;
	}

	static public function getLocations($id)
	{
		// dd($id);
		$locations = \App\Models\Locations::find($id);
		// ($locations);
		return $locations;
	}

	static public function getSuccessStories($id)
	{
		$succ_stories = \App\Models\Successstories::find($id);
		return $succ_stories;
	}
	static public function getFertilityExperts($id)
	{
		$fertilityexperts = \App\Models\Fertilityexpert::find($id);
		return $fertilityexperts;
	}
	static public function getLocationDetails($id)
	{
		$location_details = \App\Models\Locationdetailpage::find($id);
		return $location_details;
	}
	static public function getGallery($id)
	{
		$gallery = \App\Models\Gallerypage::find($id);
		return $gallery;
	}
	static public function getTestimonialVideos($id)
	{
		$succ_stories = \App\Models\Testimonialvideos::find($id);
		return $succ_stories;
	}

	static public function getBlogsByCategory($id)
	{
		$blogs = \App\Models\Post::where('cid',$id)->where('pagetype','post')->where('status','enable')->get();
		return $blogs;
	}

	static public function getBlogsById($id)
	{
		$blogs = \App\Models\Post::where('pageID',$id)->where('pagetype','post')->where('status','enable')->first();
		return $blogs;
	}

	static public function getSituations($id)
	{
		$situation = \App\Models\Situation::find($id);
		return $situation;
	}
