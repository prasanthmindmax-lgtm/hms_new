...
class NursingPageController extends Controller
{
	protected $layout = "layouts.main";
	protected $data = array();	
	public $module = 'nursingpage';
	static $per_page	= '10';

	public function __construct()
	{		
		parent::__construct();
		$this->model = new NursingPage();	
	...
	function create( Request $request , $id =0 ) 
	{
		...
		$nursingData = NursingPage::get();

		if(count($nursingData)>0){
		...
		switch ($task)
		{
			default:
				...
				if(!is_null($request->file('banner_image')))
				{
					...
					$destinationPath = './uploads/nursing/';
	...
	public static function display(  )
	{
		...
		$model  = new NursingPage();
		$info = $model::makeInfo('nursingpage');
		$data = array(
