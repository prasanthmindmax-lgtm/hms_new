...
class FacilitiespageController extends Controller {
	...
	public $module = 'facilitiespage';
	static $per_page	= '10';

	public function __construct()
	{		
		parent::__construct();
		$this->model = new Facilitiespage();	
		
		$this->info = $this->model->makeInfo( $this->module);	
		$this->data = array(
			'pageTitle'	=> 	$this->info['title'],
			'pageNote'	=>  $this->info['note'],
			'pageModule'=> 'facilitiespage',
		...
		switch ($task)
		{
			default:
				...
				if(!is_null($request->file('banner_image')))
				{
					...
					$destinationPath = './uploads/facilities/';
				...
				// section 2 start
					...
					if(!is_null($request->file('image')))
					{
						...
						$destinationPath = './uploads/facilities/';
						$extension = $file->getClientOriginalExtension(); //if you need extension of the file
						$newfilename = time().rand(1000,9999).'.'.$extension;
						$uploadSuccess = $request->file('image')->move($destinationPath, $newfilename);				 
						if( $uploadSuccess ) {
							$section_2['image'] = $newfilename; 
						}
					}	

					else{
						$section_2['image'] = $request->img_name;
					}

					$section_2['description'] = $request->sec2_description;
	...
	public static function display(  )
	{
		...
		$model  = new Facilitiespage();
		$info = $model::makeInfo('facilitiespage');
		$data = array(
		...
		if($mode == 'view')
		{
			...
			if($row)
			{
				...
				$data['id'] = $id;
				return view('facilitiespage.public.view',$data);			
			}			
		} 
		else {
			...
			return view('facilitiespage.public.index',$data);	
		}
