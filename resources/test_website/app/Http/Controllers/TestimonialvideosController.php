...
class TestimonialvideosController extends Controller {

	protected $layout = "layouts.main";
	protected $data = array();	
	public $module = 'testimonialvideos';
	static $per_page	= '10';

	public function __construct()
	{		
		parent::__construct();
		$this->model = new Testimonialvideos();	
		
		$this->info = $this->model->makeInfo( $this->module);	
		$this->data = array(
			'pageTitle'	=> 	$this->info['title'],
			'pageNote'	=>  $this->info['note'],
			'pageModule'=> 'testimonialvideos',
			...
			default:
				...
				if ($validator->passes()) 
				{
					...
					if(!is_null($request->file('image')))
					{
						...
						$destinationPath = './uploads/testimonialvideos/';
						$extension = $file->getClientOriginalExtension(); //if you need extension of the file
						$newfilename = time().'-'.rand(1000,10000).'.'.$extension;
	...
	public static function display(  )
	{
		...
		$model  = new Testimonialvideos();
		$info = $model::makeInfo('testimonialvideos');
		$data = array(
		...
		if($mode == 'view')
		{
			...
			if($row)
			{
				...
				$data['id'] = $id;
				return view('testimonialvideos.public.view',$data);			
			}			
		} 
		else {
			...
			return view('testimonialvideos.public.index',$data);	
		}
