...
class BloglistpageController extends Controller {
	...
	public $module = 'bloglistpage';
	static $per_page	= '10';

	public function __construct()
	{		
		parent::__construct();
		$this->model = new Bloglistpage();	
		
		$this->info = $this->model->makeInfo( $this->module);	
		$this->data = array(
			'pageTitle'	=> 	$this->info['title'],
			'pageNote'	=>  $this->info['note'],
			'pageModule'=> 'bloglistpage',
	...
	function create( Request $request , $id =0 ){
		...
		$bloglist_data = Bloglistpage::get();
			if(count($bloglist_data) == 0) {
				$this->data['row'] = $this->model->getColumnTable( $this->info['table']); 
				$this->data['id'] = '';
				return view($this->module.'.form',$this->data);
			}
			else{
				$id = $bloglist_data[0]['id'];
	...
	public static function display(  )
	{
		...
		$model  = new Bloglistpage();
		$info = $model::makeInfo('bloglistpage');
		$data = array(
		...
		if($mode == 'view')
		{
			...
			if($row)
			{
				...
				$data['id'] = $id;
				return view('bloglistpage.public.view',$data);			
			}			
		} 
		else {
			...
			return view('bloglistpage.public.index',$data);	
		}
