...
class CareerspageController extends Controller {
	...
	public $module = 'careerspage';
	static $per_page	= '10';

	public function __construct()
	{		
		parent::__construct();
		$this->model = new Careerspage();	
		
		$this->info = $this->model->makeInfo( $this->module);	
		$this->data = array(
			'pageTitle'	=> 	$this->info['title'],
			'pageNote'	=>  $this->info['note'],
			'pageModule'=> 'careerspage',
	...
	function create( Request $request , $id =0 ) 
	{
		...
		$careerpage_data = Careerspage::get();
		if(count($careerpage_data) == 0) {
				$this->data['row'] = $this->model->getColumnTable( $this->info['table']); 
				$this->data['id'] = '';
				return view($this->module.'.form',$this->data);
			}
			else{
				$id = $careerpage_data[0]['id'];
	...
	public static function display(  )
	{
		...
		$model  = new Careerspage();
		$info = $model::makeInfo('careerspage');
		$data = array(
		...
		if($mode == 'view')
		{
			...
			if($row)
			{
				...
				$data['id'] = $id;
				return view('careerspage.public.view',$data);			
			}			
		} 
		else {
			...
			return view('careerspage.public.index',$data);	
		}
