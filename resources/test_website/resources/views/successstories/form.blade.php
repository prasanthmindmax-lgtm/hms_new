...
<div class="card">
	...
	{!! Form::open(array('url'=>'successstories?return='.$return, 'class'=>'form-horizontal  validated sximo-form','files' => true ,'id'=> 'FormTable' )) !!}
				...
				{!! Form::hidden('id', $row['id']) !!}					
						 ...
						 >{{ $row['description'] }}</textarea> 
				           ...
				           >{{ $row['youtube_url'] }}</textarea> 
									  ...
									  <div class="form-group row  " >
										<label for="Youtube URL" class=" control-label col-md-4 "> Embedded URL </label>
										<div class="col-md-8">
										  <textarea name='embedded_url' rows='5' id='embedded_url' class='form-control form-control-sm '  
				           >{{ $row['embedded_url'] }}</textarea> 
										 </div> 
										 
									  </div> 
									  <div class="form-group row  " >
										<label for="Youtube URL" class=" control-label col-md-4 "> More Videos URL </label>
										<div class="col-md-8">
										  <textarea name='more_video_url' rows='5' id='more_video_url' class='form-control form-control-sm '  
				           >{{ $row['more_video_url'] }}</textarea> 
										 </div> 
									  ...
									  <div class="form-group row  " >
										...
										@if(isset($row['image'])) {!! SiteHelpers::showUploadedFile( $row['image'],"/uploads/successstories/") !!} @endif
										</div>
										<input  type='hidden' name='img_name' id='img_name' class='form-control form-control-sm' value="@if(isset($row['image'])) {{ $row['image'] }}  @endif"/> 
										</div> 
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("successstories/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
