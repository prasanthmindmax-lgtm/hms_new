    ...
    <div class="qd-card">
      ...
      <div class="qd-header">
        <div class="qd-header-title">
          <i class="bi bi-diagram-3"></i> Expense Type Master
        </div>
        <div class="qd-header-actions">
          <button class="btn btn-primary btn-sm expense_type_new">
            <i class="bi bi-plus-lg me-1"></i>New Expense Type
          </button>
        </div>
      </div>
      ...
      <div class="qd-search-row">
        <div class="qd-search-wrap">
          ...
          <input type="text" id="tableSearch" placeholder="Search expense type name...">
        </div>
      </div>
      ...
      <div class="qdt-wrap">
        <table class="qdt-table" id="mainTable">
          ...
          <tbody>
            @forelse ($types as $type)
              <tr class="qdt-row type-row" data-id="{{ $type->id }}" data-type='@json($type)'>
                <td class="qdt-td-check"><input type="checkbox" class="row-check"></td>
                <td style="font-weight:600;color:#1f2937;">{{ $type->name }}</td>
                <td>
                  @if($type->is_active == 1)
            ...
            @empty
              <tr><td colspan="4" class="text-center py-5 text-muted">
                <i class="bi bi-inbox" style="font-size:2rem;display:block;margin-bottom:8px;"></i>No expense types found
              </td></tr>
            @endforelse
          </tbody>
        </table>
        ...
        <div class="qd-pagination d-flex justify-content-between align-items-center mt-2 px-1">
          <div>{{ $types->links('pagination::bootstrap-4') }}</div>
...
<div class="sm-modal" id="expenseTypeModal">
  <div class="sm-modal-box">
    <div class="sm-modal-header">
      <div class="sm-modal-title"><i class="bi bi-diagram-3" style="color:#4f6ef7;"></i> Expense Type</div>
      <button class="sm-modal-close close-modal">&times;</button>
    </div>
    <input type="hidden" class="type_id" id="type_id">
    <div class="sm-form-row">
      <div class="sm-form-group">
        <label>Name <span style="color:red">*</span></label>
        <input type="text" class="form-control name" placeholder="e.g. Food, Travel" autocomplete="off">
      </div>
    </div>
    <div class="sm-form-row">
        <div class="sm-form-group">
    ...
    <div class="sm-modal-footer">
      ...
      <button class="sm-btn-primary expense_type_save">Save</button>
    </div>
  </div>
</div>
...
$(document).ready(function () {
  ...
  function openModal() {
    $('#expenseTypeModal').addClass('show');
    $('#modalOverlay').addClass('show');
    $('body').css('overflow','hidden');
  }

  function closeModal() {
    $('#expenseTypeModal').removeClass('show');
    $('#modalOverlay').removeClass('show');
    $('body').css('overflow','auto');
  }

  $('.expense_type_new').on('click', function () {
    $('#type_id').val('');
    $('.name').val('');
    $('.description').val('');
    $('#is_active').val('1');
  ...
  $(document).on('click', '.expense_type_save', function (e) {
    e.preventDefault();
    const fd = new FormData();
    fd.append('id', $('#type_id').val());
    fd.append('name', $('.name').val());
    fd.append('description', $('.description').val());
    fd.append('is_active', $('#is_active').val());
    $.ajax({
      url:'{{ route("superadmin.expensetype.store") }}',
      type:'POST',
      data:fd,
      processData:false,
      contentType:false,
