...
<style>
    .gst_seperate span { margin:0; font-size:10px; }
</style>
            ...
            <div class="qd-card">
                ...
                <div class="qd-header">
                    ...
                    <div class="qd-header-actions">
                        ...
                        <a href="{{ route('superadmin.getquotationcreate') }}" class="btn btn-primary btn-sm">
                            <i class="bi bi-plus-lg me-1"></i>New Quotation
                        </a>
                        <div class="dropdown">
                            <button class="btn btn-light btn-sm" data-bs-toggle="dropdown">&#x22EE;</button>
                            <ul class="dropdown-menu dropdown-menu-end">
                                <li><a href="#" class="dropdown-item" data-bs-toggle="modal" data-bs-target="#importModal">
                                    <i class="bi bi-upload me-2"></i>Import Quotations
                                </a></li>
                            </ul>
                        </div>
                    </div>
                ...
                <div class="qd-stats" id="statsSection">
                    <div class="qd-stat-card qd-stat-blue qd-stat-active" data-stat-filter="" title="Show all">
                        ...
                        <div class="qd-stat-body">
                            <div class="qd-stat-label">Total Quotations</div>
                ...
                <div class="qd-search-row">
                    <div class="qd-search-wrap">
                        ...
                        <input type="text" class="universal_search" placeholder="Search quotation...">
                    </div>
                </div>
                ...
                <div class="qd-table-wrap">
                    <div id="quotation-body">
                        @include('vendor.partials.table.quotation_rows',['quotationlist' => $quotationlist, 'perPage' => $perPage])
                    </div>
                </div>
            ...
            <div class="modal-dialog">
              <div class="modal-content">
                  <div class="modal-header">
                    <h5 class="modal-title" id="importModalLabel">Import Quotations</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                  </div>
                  <div class="modal-body">
                    <p>Step 1: Download the Excel Template</p>
                    <a href="{{ url('/download-quotation-template') }}" class="btn btn-success mb-3">
                      Download Template
                    </a>
...
$(document).ready(function () {
  ...
  $(document).on('click', '.approver', function (e) {
      ...
      $.ajax({
          url: '{{ route("superadmin.QuotationApprover") }}',
          method: "GET",
  ...
  $(document).on('click', '.import-btn', function (e) {
    ...
    //     alert(billId);
    //     window.location.href = "{{ route('superadmin.getquotationcreate') }}" + "?id=" + billId + "&type=edit";
    // });
    ...
    // });
     $(document).on('click', '.edit-btn', function () {
      ...
      //     "{{ route('superadmin.getquotationcreate') }}" +
      //     "?id=" + billId +
      //     "&type=edit" +
      //     "&redirect_page=" + page;
    ...
    $(document).on('click', '.clone-btn', function () {
        const billId = $('#billDetailModal').data('bill-id');
        window.location.href = "{{ route('superadmin.getquotationcreate') }}" + "?id=" + billId + "&type=clone";
    });
     $(document).on('click', '.pdf-btn', function() {
      const billId = $('#billDetailModal').data('bill-id');
      // Make AJAX request
  ...
  // jQuery/vanilla fallback: will hide modal for Bootstrap 4, Bootstrap 5, or just remove backdrop if needed
    ...
    function populateModal(data) {
        ...
        $('#tds_amount').text(formatCurrency(data.quotation.tax_amount));

        $('#gst_amount').text(formatCurrency(data.quotation.gst_amount));
        $('#grand-total').text(formatCurrency(data.grand_total));
        $('#notes').text(data.note || 'No notes');

        $('.zoho-total-row.export-row').remove();
        if (data.quotation.export_name !== null) {
          const exportRow = `
            <div class="zoho-total-row export-row">
              <span>${data.quotation.export_name}</span>
              <span id="tds_amount">${formatCurrency(data.quotation.export_amount)}</span>
            </div>
          `;
        ...
        const itemsHtml = data.items.map(item => `
            <tr>
                ...
                <td>${item.account || 'No description'}</td>
                <td>
        ...
        // Clear containers
        $('.upload_doc').empty();
        let uploads = data.quotation.documents;
    ...
    //           return;
    //       }
    //       wrapper.find('.zone-search-input').val(selectedText).trigger('change');
  ...
  // $(document).on('click', '.zone-search-input', function (e) {
  //   e.stopPropagation();
  //   const $dropdown = $(this).siblings('.dropdown-menu.tax-dropdown');
  //   $('.dropdown-menu.tax-dropdown').not($dropdown).hide();
  //   $dropdown.toggle();
  // });

  // // Search filter
  // $(document).on('keyup', '.zone-inner-search', function () {
  //   const searchVal = $(this).val().toLowerCase();
  //   $(this).closest('.dropdown-menu').find('.zone-list div').each(function () {
  //     const text = $(this).text().toLowerCase();
  //     $(this).toggle(text.indexOf(searchVal) > -1);
  //   });
  // });
  ...
  // $(document).on('click', '.zone-list div', function () {
  //   $(this).toggleClass('selected');
  //   updateZoneSelection($(this).closest('.tax-dropdown'));
  // });

  // // Select all
  // $(document).on('click', '.select-all', function () {
  //   const $dropdown = $(this).closest('.tax-dropdown');
  //   $dropdown.find('.zone-list div').addClass('selected');
  //   $('.zone_id').trigger('click');
  //   updateZoneSelection($dropdown);
  // });

  // // Deselect all
  // $(document).on('click', '.deselect-all', function () {
  //   const $dropdown = $(this).closest('.tax-dropdown');
  //   $dropdown.find('.zone-list div').removeClass('selected');
  //   $('.zone_id').trigger('click');
  //   updateZoneSelection($dropdown);
  // });

  // // Update selected items
  // function updateZoneSelection($dropdown) {
  //   const selectedItems = [];
  //   const selectedIds = [];

  //   $dropdown.find('.zone-list div.selected').each(function () {
  //     selectedItems.push($(this).text().trim());
  //     selectedIds.push($(this).data('id'));
  //   });

  //   const wrapper = $dropdown.closest('.tax-dropdown-wrapper');
  //   wrapper.find('.zone-search-input').val(selectedItems.join(', '));
  //   wrapper.find('.zone_id').val(selectedIds.join(','));
  //   $('.zone_id').trigger('click');
  // }
  ...
  //             return;
  //         }
  //         wrapper.find('.branch-search-input').val(selectedText).trigger('change');
  //         wrapper.find('.branch_id').val(selectedid).trigger('click');
  ...
  //           wrapper.find('.company_id').val(selectedid).trigger('click');
  ...
  //           wrapper.find('.vendor_id').val(selectedid).trigger('click');
  ...
  //       });
      ...
      //   function loadQuotations(page = 1, perPage = $('#per_page').val()) {
      //       $.ajax({
      //           url: '{{ route("superadmin.getquotation") }}',
      //           type: "GET",
      //           data: {
      //               per_page: perPage,
      //               page: page,
      ...
      //       loadQuotations(page, perPage);
      //   });

      //     // Change per_page without reload
      //     $(document).on('change', '#per_page', function () {
      //         let perPage = $(this).val();
      //         loadQuotations(1, perPage); // Always go to first page when per_page changes
      //     });
      // });
 ...
 $(document).ready(function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data && typeof data === 'object' && data.html !== undefined) {
                        // JSON response with html + stats
                        $("#quotation-body").html(data.html);
                        if (data.stats) {
        ...
        $("#filter-summary").on('click', '#clear-all', function () {
            ...
            $('.zone-search-input, .branch-search-input, .company-search-input, .vendor-search-input,.nature-search-input,.status-search-input,.state-search-input,.creator-search-input').val('');
            $('.creator-list div').removeClass('selected');
      ...
      $(document).on('click', '.qd-stat-card[data-stat-filter]', function () {
          ...
          // Sync the status dropdown display
          $('.status-search-input').val(filters.status_name || '');
