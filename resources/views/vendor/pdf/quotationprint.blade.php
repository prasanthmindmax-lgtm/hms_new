...
<head>
    <meta charset="utf-8">
    <title>Quotation Order - {{ $quotation->quotation_gen_no ?? '000082' }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    ...
    $company = $quotation->TblCompany; // may be null
...
<img src="{{ $logoBase64 }}" class="logo-image">
    ...
    <div class="logo-section">
        <img src="{{ public_path('uploads/vendor/company/' . $quotation->TblCompany->logo_upload) }}"
    ...
    <div class="top-section">
        <!-- Left Column -->
        <div class="left-info">
            <div class="company-name">{{ $quotation->TblCompany->company_name ?? "Dr.Aravind's IVF Private Limited" }}</div>
            <div class="company-details">
                {{ $quotation->TblCompany->state ?? 'Tamil Nadu' }}<br>
                {{ $quotation->TblCompany->country ?? 'India' }}<br>
                {{ $quotation->TblCompany->phone ?? '+91 90 2012 2012' }}<br>
                {{ $quotation->TblCompany->email ?? 'info@draravindsivf.com' }}
            </div>

            <div class="vendor-section">
                <div class="section-label">Vendor Address</div>
                <div class="vendor-name">{{ $quotation->Tblvendor->display_name ?? '-' }}</div>
                <div class="vendor-details">
                    {{ $quotation->TblBilling->address ?? '-' }}<br>
                    {{ $quotation->TblBilling->city ?? '-' }}<br>
                    {{ $quotation->TblBilling->zip_code ?? '-' }} {{ $quotation->TblBilling->state ?? '-' }}<br>
            ...
            <div class="delivery-section">
                ...
                <div class="vendor-details">{{ $quotation->delivery_address ?? '-' }}</div>
            </div>
        </div>

        <!-- Right Column -->
        <div class="right-info">
            <div class="quotation-title-main">Quotation Order</div>
            <div class="quotation-number">{{ $quotation->quotation_gen_no ?? '-' }}</div>
            <div class="quotation-number">{{ $quotation->quotation_no ?? '-' }}</div>
            <div class="balance-label">Balance Due</div>
            <div class="balance-amount">₹{{ number_format($quotation->balance_amount ?? 50000.00, 2) }}</div>

            <table class="order-info-table">
                <tr>
                    <td>Order Number :</td>
                    <td>{{ $quotation->order_number ?? '000082' }}</td>
                </tr>
                <tr>
                    <td>Quotation Date :</td>
                    <td>{{ $quotation->bill_date ?? '15/10/2025' }}</td>
                </tr>
                <tr>
                    <td>Due Date :</td>
                    <td>{{ $quotation->due_date ?? '13/11/2025' }}</td>
                </tr>
                <tr>
                    <td>Terms :</td>
                    <td>{{ $quotation->payment_terms ?? 'Due on Receipt' }}</td>
                </tr>
            </table>
        </div>
    </div>
    ...
    <table class="items-table">
        ...
        <tbody>
            @if(isset($quotation->BillLines) && count($quotation->BillLines) > 0)
                @foreach($quotation->BillLines as $index => $line)
                    <tr>
    ...
    <div class="summary-section">
        ...
        @endphp
            ...
            <div class="summary-row">
                ...
                <div class="summary-value sub_total">₹{{ number_format($quotation->sub_total_amount ?? 944.00, 2) }}</div>
            </div>
            ...
            <div class="summary-row">
                ...
                <div class="summary-value">₹{{ number_format($quotation->discount_amount ?? 0.00, 2) }}</div>
            </div>

            <div class="summary-row">
                <div class="summary-label">TDS(-)</div>
                <div class="summary-value">₹{{ number_format($quotation->tax_amount ?? 0.00, 2) }}</div>
            </div>

            <div class="summary-row total-row">
                <div class="summary-label">TOTAL</div>
                <div class="summary-value">₹{{ number_format($quotation->grand_total_amount ?? 944, 0) }}</div>
            </div>

            @if(isset($quotation->export_name) && $quotation->export_name !== null)
                <div class="summary-row">
                    <div class="summary-label">{{ $quotation->export_name }}</div>
                    <div class="summary-value">₹{{ number_format($quotation->export_amount ?? 0.00, 2) }}</div>
                </div>
            @endif

            <div class="summary-row payment-row">
                <div class="summary-label">Payments Made</div>
                <div class="summary-value">(-) ₹{{ number_format($quotation->partially_payment ?? 0.00, 2) }}</div>
            </div>

            <div class="balance-due-final">
                Balance Due : ₹{{ number_format($quotation->balance_amount ?? 944.00, 2) }}
            </div>
        </div>
    </div>
