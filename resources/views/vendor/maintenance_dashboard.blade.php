    ...
    <div class="pc-content">
      ...
      <div class="qd-card">
        ...
        <div class="qd-stats" id="statsSection">
          <div class="qd-stat-card qd-stat-blue qd-stat-active" data-stat-filter="" title="All Maintenance Bills">
            <div class="qd-stat-icon"><i class="bi bi-tools"></i></div>
            <div class="qd-stat-body">
              <div class="qd-stat-label">Total Maintenance</div>
        ...
        <div class="qd-table-wrap">
          <div id="bill-body">
            @include('vendor.partials.table.bill_rows', ['billlist' => $billlist, 'perPage' => $perPage, 'limit_access' => $limit_access ?? null, 'billTableMode' => 'maintenance'])
          </div>
        </div>
  ...
  <div class="asset-panel" id="assetDetailPanel">
    ...
    <div class="asset-panel-body">
      ...
      <div class="ap-card">
        ...
        <div class="ap-grid">
          <div class="ap-item">
            <div class="ap-lbl">Bill Number</div>
            <div class="ap-val" id="ap-bill-no">—</div>
          </div>
          <div class="ap-item">
            <div class="ap-lbl">Order Number</div>
            <div class="ap-val" id="ap-order-no">—</div>
          </div>
          <div class="ap-item">
            <div class="ap-lbl">Bill Date</div>
            <div class="ap-val" id="ap-bill-date">—</div>
          </div>
          <div class="ap-item">
            <div class="ap-lbl">Due Date</div>
            <div class="ap-val" id="ap-due-date">—</div>
          </div>
          <div class="ap-item">
            <div class="ap-lbl">Payment Terms</div>
            <div class="ap-val" id="ap-payment-terms">—</div>
          </div>
          <div class="ap-item">
            <div class="ap-lbl">Zone / Branch</div>
            <div class="ap-val" id="ap-zone-branch">—</div>
          </div>
        </div>
      </div>
      ...
      <div class="ap-card">
        ...
        <div class="ap-grid">
          <div class="ap-item">
            <div class="ap-lbl">Vendor Name</div>
            <div class="ap-val ap-vendor-name">—</div>
          </div>
          <div class="ap-item">
            <div class="ap-lbl">Phone</div>
            <div class="ap-val ap-vendor-phone">—</div>
          </div>
  ...
  <script>
    $(document).ready(function() {
      ...
      function loadBill(page = 1, perPage = $('#per_page').val()) {
        $.ajax({
          url: '{{ route("superadmin.getmaintenance") }}',
          type: 'GET',
          data: {
            per_page: perPage,
            page,
            ...
            if (typeof data === 'object' && data.html !== undefined) {
              ...
              if (data.stats) {
                $.each(data.stats, function(key, val) {
                  ...
                  if (key === 'total_amount') {
                    $el.text('₹' + parseFloat(val).toLocaleString('en-IN', {
      ...
      $('#assetDetailPanel').on('click', function(e) {
        e.stopPropagation();
      });
      ...
      $(document).on('click', '.print-btn', function() {
        ...
        $.ajax({
          ...
          success: function(r) {
            $('#pdfFrame').attr('src', URL.createObjectURL(new Blob([r], {
              type: 'application/pdf'
            })));
      ...
      $(document).on('click', '.modal-close-fallback', function(e) {
        ...
        try {
          if (typeof bootstrap !== 'undefined') {
            var m = bootstrap.Modal.getInstance(document.getElementById('pdfPreviewModal')) || new bootstrap.Modal(document.getElementById('pdfPreviewModal'));
            m.hide();
            return;
          }
        } catch (err) {}
      ...
      $(document).on('click', '.documentclk', function() {
        ...
        fileArray.forEach(file => {
          ...
          views += `<button style="font-size:11px;" type="button" class="btn btn-primary pdf-doc-btn mb-1" data-filepath="${file}">${fn}</button>`;
        });
        $('#image_pdfs').html(views);
      });
      $(document).on('click', '.pdf-doc-btn', function() {
        $('.pdf-doc-btn').removeClass('active');
      ...
      $('#submit-neft-datas').click(function(e) {
        ...
        if ($('#created_by').val() === '') {
          $('.error_created_by').text('Enter Creator Name');
          isValid = false;
        }
        if ($('#vendor-search').val() === '') {
          $('.error_special').text('Select Vendor');
          isValid = false;
        }
        if ($('#nature_payment').val() === '') {
          $('.error_nature_payment').text('Enter Nature Payment');
          isValid = false;
        }
        if ($('#invoice_amount').val() === '') {
          $('.error_invoice_amount').text('Enter Invoice Amount');
          isValid = false;
        }
        ...
        if (!panPattern.test($('#pan_number').val().toUpperCase())) {
        ...
        $.ajax({
          ...
          success: function(r) {
            ...
            if (r.success) $('#NEFTModal').fadeOut();
      ...
      $('input[type="file"]').on('change', function() {
        ...
        const $prev = $(`#preview_${id}`);
        const files = Array.from(this.files);
        previewFiles[id] = files;
        $prev.empty();
      ...
      $(document).on('click', '.remove-file-btn', function() {
        ...
        previewFiles[id].forEach(f => dt.items.add(f));
        $(`#${id}`)[0].files = dt.files;
        const $prev = $(`#preview_${id}`);
        $prev.empty();
