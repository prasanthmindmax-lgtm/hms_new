    ...
    <div class="qd-card">
      ...
      <div class="qd-header">
        <div class="qd-header-title">
          <i class="bi bi-diagram-3"></i> Vendor Type
        </div>
          ...
          <tbody>
            ...
            @empty
              <tr><td colspan="5" class="text-center py-5 text-muted">
                <i class="bi bi-inbox" style="font-size:2rem;display:block;margin-bottom:8px;"></i>No vendor types found
              </td></tr>
            @endforelse
          </tbody>
        </table>
...
<div class="sm-modal" id="newgstModal">
  <div class="sm-modal-box">
    <div class="sm-modal-header">
      <div class="sm-modal-title"><i class="bi bi-diagram-3" style="color:#4f6ef7;"></i> Vendor Type</div>
      <button class="sm-modal-close close-modal">&times;</button>
    </div>
    <input type="hidden" class="vendor_type_id" id="vendor_type_id">
    <div class="sm-form-row">
      <div class="sm-form-group">
        <label>Name <span style="color:red">*</span></label>
        <input type="text" class="form-control name" placeholder="e.g. Supplier, Contractor" autocomplete="off">
      </div>
    </div>
    <div class="sm-form-row">
      <div class="sm-form-group">
    ...
    <div class="sm-modal-footer">
      ...
      <button class="sm-btn-primary vendor_type_save">Save</button>
    </div>
  </div>
</div>
...
$(document).ready(function () {
  ...
  $('.gst_new').on('click', function () { $('#vendor_type_id,.name').val(''); $('#description').val(''); openModal(); });
  $(document).on('click', '.edit-btn', function () {
    const d = $(this).closest('tr').data('nature');
    $('#vendor_type_id').val(d.id); $('.name').val(d.name); $('#description').val(d.description);
    openModal();
  });
  $(document).on('click', '.close-modal, #modalOverlay', closeModal);

  $(document).on('click', '.vendor_type_save', function (e) {
    e.preventDefault();
    const fd = new FormData();
    fd.append('id', $('.vendor_type_id').val()); fd.append('name', $('.name').val()); fd.append('description', $('#description').val());
    $.ajax({
      url:'{{ route("superadmin.getvendortypesave") }}', type:'POST', data:fd, processData:false, contentType:false,
