...
$pdf->SetXY(15, 30);
$pdf->Cell(0, 0, $bill->TblCompany->company_name ?? "Aravind's IVF", 0, 1);

$pdf->SetFont('dejavusans', '', 9);
$pdf->SetXY(15, 35);
$pdf->MultiCell(70, 5,
    ($bill->TblCompany->state ?? 'Tamil Nadu') . "\n" .
    ($bill->TblCompany->country ?? 'India') . "\n" .
    ($bill->TblCompany->phone ?? '+91 90 2012 2012') . "\n" .
...
$pdf->SetXY(150, 15);
$pdf->Cell(0, 0, 'BILL', 0, 1, 'R');

$pdf->SetFont('dejavusans', '', 10);
$pdf->SetXY(150, 22);
$pdf->Cell(0, 0, 'Bill# ' . ($bill->bill_number ?? '-'), 0, 1, 'R');
...
$pdf->Cell(0, 0, '₹' . number_format($bill->balance_amount ?? '-'), 0, 1, 'R');
$pdf->SetTextColor(0, 0, 0); // reset to black

// Bill From Section
$pdf->SetXY(15, 55);
$pdf->SetFont('dejavusans', '', 9);
$pdf->Cell(0, 0, 'Bill From', 0, 1);

$pdf->SetFont('dejavusans', 'B', 9);
$pdf->SetXY(15, 60);
$pdf->Cell(0, 0, $bill->Tblvendor->display_name, 0, 1);

$pdf->SetFont('dejavusans', '', 9);
$pdf->SetXY(15, 65);
$pdf->MultiCell(70, 5,
    ($bill->TblBilling->address) . "\n" .
    ($bill->TblBilling->city) . "\n" .
    ($bill->TblBilling->zip_code) . ' ' . ($bill->TblBilling->state) . "\n" .
    ($bill->TblBilling->country),
    0, 'L', false);
...
$pdf->Cell($col7, 8, 'GST Amount', 0, 0, 'C', true);
...
while ($currentItem < $itemCount) {
    ...
    if ($y + $maxHeight > 270) { // Reduced threshold to account for row height
        $pdf->AddPage();
...
// Summary Section - REST OF YOUR ORIGINAL CODE REMAINS THE SAME
if ($y > 170) {
    $pdf->AddPage();
    $y = 30;
}
...
$pdf->Cell(30, 7, number_format($bill->sub_total_amount, 2), 0, 1, 'R');
$summaryY += 7;
...
$pdf->SetFont('dejavusans', 'B', 10);
$pdf->Cell(30, 7, '₹' . number_format($bill->discount_amount ?? '-'), 0, 1, 'R');

$pdf->SetXY(140, $summaryY + 14);
$pdf->Cell(30, 7, 'TDS(-)', 0, 0, 'L');
$pdf->SetFont('dejavusans', 'B', 10);
$pdf->Cell(30, 7, '₹' . number_format($bill->tax_amount ?? '-'), 0, 1, 'R');

$pdf->SetXY(140, $summaryY + 21);
$pdf->Cell(30, 7, 'TOTAL', 0, 0, 'L');
$pdf->SetFont('dejavusans', 'B', 10);
$pdf->Cell(30, 7, '₹' . number_format($bill->grand_total_amount ?? '-'), 0, 1, 'R');

if($bill->export_name !== null){
    $pdf->SetXY(140, $summaryY + 28);
    $pdf->Cell(30, 7, $bill->export_name , 0, 0, 'L');
    $pdf->SetFont('dejavusans', 'B', 10);
    $pdf->Cell(30, 7, '₹' . number_format($bill->export_amount ?? 100000.00, 2), 0, 1, 'R');
}
...
if($bill->export_name === null){
    ...
    $pdf->Cell(30, 7, '(-) ₹' . number_format($bill->partially_payment ?? 50000.00, 2), 0, 1, 'R');
}else{
    $pdf->SetFont('dejavusans', '', 9);
    $pdf->SetTextColor(255, 0, 0);
...
if($bill->export_name === null){
    ...
    $pdf->SetFont('dejavusans', 'B', 10);
    $pdf->SetXY(140, $summaryY + 35);
    $pdf->Cell(60, 8, 'Balance Due : ₹' . number_format($bill->balance_amount ?? 50000.00, 2), 0, 1, 'C', true);
}else{
    $pdf->SetFillColor(240, 240, 240);
    $pdf->SetTextColor(0, 0, 0);
    $pdf->SetFont('dejavusans', 'B', 10);
