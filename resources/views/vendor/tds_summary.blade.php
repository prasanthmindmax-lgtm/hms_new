...
.tds-panel {
    ...
    transition: right .3s ease; border-left: 3px solid #4361ee;
    overflow: hidden;
}
.tds-panel.show { right: 0; }
.tds-panel-backdrop { display:none; position:fixed; inset:0; background:rgba(0,0,0,.35); z-index:9998; }
.tds-panel-backdrop.show { display:block; }
.tp-header { display:flex; align-items:center; justify-content:space-between; padding:14px 20px; border-bottom:1px solid #e9ecef; background:#f8f9fa; flex-shrink:0; }
...
.tp-section { background:#fff; border:1px solid #e9ecef; border-radius:8px; padding:14px; margin-bottom:14px; }
.tp-section-title { font-size:11px; font-weight:700; text-transform:uppercase; letter-spacing:.5px; color:#6c757d; margin-bottom:10px; padding-bottom:6px; border-bottom:1px solid #f0f0f0; }
.tp-row { display:flex; justify-content:space-between; padding:5px 0; border-bottom:1px solid #f8f9fa; font-size:13px; }
.tp-row:last-child { border-bottom:none; }
.tp-label { color:#6c757d; font-weight:500; }
.tp-val { color:#1a1a2e; font-weight:600; text-align:right; max-width:60%; }
.tp-items-table { width:100%; border-collapse:collapse; font-size:12px; }
.tp-items-table th { background:#f8f9fa; color:#6c757d; font-size:10px; text-transform:uppercase; padding:7px 10px; border-bottom:2px solid #e9ecef; }
.tp-items-table td { padding:7px 10px; border-bottom:1px solid #f5f5f5; }
.tp-totals { display:flex; flex-direction:column; gap:4px; font-size:13px; }
.tp-total-row { display:flex; justify-content:space-between; padding:4px 0; }
.tp-total-row.grand { font-weight:700; font-size:14px; border-top:2px solid #e9ecef; padding-top:8px; color:#4361ee; }
.tp-badge-pending { background:#fff3cd; color:#856404; padding:3px 10px; border-radius:20px; font-size:12px; font-weight:600; }
.tp-badge-paid    { background:#d1e7dd; color:#0a3622; padding:3px 10px; border-radius:20px; font-size:12px; font-weight:600; }
</style>
    ...
    <div class="qd-card">
      ...
      <div class="qd-header">
        <div class="qd-header-title">
          <i class="bi bi-receipt-cutoff"></i> TDS Summary
        </div>
        <div class="qd-header-actions">
          ...
          <div class="dropdown">
            ...
            <ul class="dropdown-menu dropdown-menu-end">
              <li><a href="#" class="dropdown-item tds-summary-export" data-format="xlsx"><i class="bi bi-file-earmark-excel me-2"></i>Download XLSX</a></li>
              <li><a href="#" class="dropdown-item tds-summary-export" data-format="csv"><i class="bi bi-file-earmark-text me-2"></i>Download CSV</a></li>
            </ul>
          </div>
        </div>
      </div>
      ...
      <div class="qd-stats" id="statsSection">
        <div class="qd-stat-card qd-stat-blue qd-stat-active" data-stat-filter="" title="Show all TDS bills">
        ...
        <div class="qd-stat-card qd-stat-orange" data-stat-filter="month" title="This Month TDS">
        ...
        <div class="qd-stat-card qd-stat-red" data-stat-filter="pending" title="Filter: Pending TDS">
      ...
      <div class="qd-table-wrap" id="tds-body">
        @include('vendor.partials.table.tds_summary_rows', ['billlist' => $billlist, 'tdsSummaryCalculation' => $tdsSummaryCalculation, 'perPage' => $perPage])
      </div>
...
{{-- ── TDS Detail Panel ── --}}
<div class="tds-panel" id="tdsDetailPanel">
  <div class="tp-header">
    <div class="tp-title" id="tp-bill-number">Bill Details</div>
    <div class="tp-actions">
      <button class="btn btn-sm btn-outline-secondary" id="tp-print-btn"><i class="bi bi-printer"></i> Print</button>
      <button class="btn btn-sm btn-outline-primary"   id="tp-pdf-btn"><i class="bi bi-file-earmark-pdf"></i> PDF</button>
      <button class="btn btn-sm btn-outline-danger" id="closeTdsPanel"><i class="bi bi-x-lg"></i></button>
    </div>
  </div>
  <div class="tp-body">
    {{-- Bill Info --}}
    <div class="tp-section">
      <div class="tp-section-title">Bill Info</div>
      <div class="tp-row"><span class="tp-label">Bill Number</span><span class="tp-val" id="tp-bill-no">—</span></div>
      <div class="tp-row"><span class="tp-label">Order / Reference</span><span class="tp-val" id="tp-order-no">—</span></div>
      <div class="tp-row"><span class="tp-label">Bill Date</span><span class="tp-val" id="tp-bill-date">—</span></div>
      <div class="tp-row"><span class="tp-label">Due Date</span><span class="tp-val" id="tp-due-date">—</span></div>
      <div class="tp-row"><span class="tp-label">Payment Terms</span><span class="tp-val" id="tp-pay-terms">—</span></div>
      <div class="tp-row"><span class="tp-label">Bill Status</span><span class="tp-val" id="tp-bill-status">—</span></div>
      <div class="tp-row"><span class="tp-label">TDS Paid Status</span><span class="tp-val" id="tp-tds-paid-status">—</span></div>
    </div>
    {{-- Vendor --}}
    <div class="tp-section">
      <div class="tp-section-title">Vendor</div>
      <div class="tp-row"><span class="tp-label">Name</span><span class="tp-val" id="tp-vendor-name">—</span></div>
      <div class="tp-row"><span class="tp-label">Address</span><span class="tp-val" id="tp-vendor-addr">—</span></div>
    </div>
    {{-- Totals --}}
    <div class="tp-section">
      <div class="tp-section-title">Totals</div>
      <div class="tp-totals">
        <div class="tp-total-row"><span>Sub Total</span><span id="tp-sub-total">—</span></div>
        <div class="tp-total-row"><span>Discount</span><span id="tp-discount">—</span></div>
        <div class="tp-total-row" style="color:#dc3545;"><span>TDS (-)</span><span id="tp-tds-amount">—</span></div>
        <div class="tp-total-row grand"><span>Grand Total</span><span id="tp-grand-total">—</span></div>
      </div>
    </div>
    {{-- Items --}}
    <div class="tp-section">
      <div class="tp-section-title">Items &amp; Description</div>
      <table class="tp-items-table">
        ...
        <tbody id="tp-items-body"><tr><td colspan="5" class="text-center text-muted">—</td></tr></tbody>
      </table>
    </div>
    {{-- Documents --}}
    <div class="tp-section">
      <div class="tp-section-title">Documents</div>
      <div class="d-flex flex-wrap" id="tp-docs"></div>
    </div>
  </div>
</div>
<div class="tds-panel-backdrop" id="tdsBackdrop"></div>
...
{{-- ── Update (TDS Pay) Modal ── --}}
<div class="modal fade" id="updateModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog">
    <form id="updateForm">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">Update TDS Payment</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
        </div>
        <div class="modal-body">
          <input type="hidden" name="bill_tds_up_id" id="bill_tds_up_id">
          <div class="row mb-3">
            <label class="col-md-10">Pay Date*</label>
            <div class="col-md-12">
...
$(document).ready(function () {
  ...
  let statFilter = ''; // default: show all data
  ...
  $(document).on('click', '.openUpdateModal', function () {
    ...
    $('#bill_tds_up_id').val(id);
  });
  $('#updateForm').on('submit', function (e) {
    e.preventDefault();
    $.ajax({
      url: '{{ route("superadmin.billtdsupdate") }}',
      type: 'POST',
      data: {
        bill_id: $('#bill_tds_up_id').val(),
        bill_date: $('#bill_date').val(),
        challan_no: $("input[name='challan_no']").val(),
        utr_no: $("input[name='utr_no']").val(),
        quarter: $('#quarter').val(),
        _token: $('meta[name="csrf-token"]').attr('content')
      },
      success: function (res) {
        if (res.status) { toastr.success(res.message); $('#updateModal').modal('hide'); setTimeout(()=>loadTds(), 1500); }
      },
      error: function (xhr) {
  ...
  // ── Export ──
  var appliedFilters = {};
  $(document).on('click', '.tds-summary-export', function (e) {
    e.preventDefault();
    let format = $(this).data('format') || 'xlsx';
    let ids = [];
    ...
    window.location.href = "{{ route('tds.summary.download') }}?" + params.toString();
  });
  ...
  let filters = {
    ...
    state_id:'', state_name:'', section_id:'', section_name:'', universal_search:''
  };
  ...
  function loadTds(page = 1, perPage = $('#per_page').val()) {
    ...
    $.ajax({
      url: '{{ route("superadmin.gettdssummary") }}',
      type: 'GET',
      data: appliedFilters,
  ...
  $('#filter-summary').on('click', '.remove-icon', function () {
    ...
    else if (type==='section') { filters.section_id=''; filters.section_name=''; $('.tds_section_id').val(''); $('.tds-section-input').val(''); $('.tds-section-list div').removeClass('selected'); }
  ...
  $('#filter-summary').on('click', '#clear-all', function () {
    filters = { date_from:'', date_to:'', zone_id:'', zone_name:'', branch_id:'', branch_name:'', company_id:'', company_name:'', vendor_id:'', vendor_name:'', state_id:'', state_name:'', section_id:'', section_name:'', universal_search:'' };
    statFilter = '';
    $('.zone-search-input,.branch-search-input,.company-search-input,.vendor-search-input,.state-search-input,.tds-section-input').val('');
    $('.zone_id,.branch_id,.company_id,.vendor_id,.state_id,.tds_section_id').val('');
  ...
  // ── TDS Detail Panel ──
  function openTdsPanel() { $('#tdsDetailPanel').addClass('show'); $('#tdsBackdrop').addClass('show'); $('body').css('overflow','hidden'); }
  function closeTdsPanel() { $('#tdsDetailPanel').removeClass('show'); $('#tdsBackdrop').removeClass('show'); $('body').css('overflow','auto'); }
  $('#closeTdsPanel, #tdsBackdrop').on('click', closeTdsPanel);
  ...
  $(document).on('click', '.customer-row', function (e) {
    ...
    $('#tp-bill-number').text(bill.bill_number || 'Bill Details');
    $('#tp-bill-no').text(bill.bill_number || '—');
    $('#tp-order-no').text(bill.order_number || '—');
    $('#tp-bill-date').text(bill.bill_date || '—');
    $('#tp-due-date').text(bill.due_date || '—');
    $('#tp-pay-terms').text(bill.payment_terms || '—');
    $('#tp-bill-status').text(bill.bill_status || '—');

    const tdsStatus = bill.tds_paid_status || 'Not Paid';
    $('#tp-tds-paid-status').html(tdsStatus === 'Paid'
      ? `<span class="tp-badge-paid">${tdsStatus}</span>`
      : `<span class="tp-badge-pending">${tdsStatus}</span>`);

    $('#tp-vendor-name').text(bill.vendor_name || '—');
    if (addr) {
      $('#tp-vendor-addr').text(`${addr.address || ''}, ${addr.city || ''}, ${addr.state || ''} - ${addr.zip_code || ''}`);
    }
    $('#tp-sub-total').text(fmt(bill.sub_total_amount));
    $('#tp-discount').text(fmt(bill.discount_amount));
    $('#tp-tds-amount').text(fmt(bill.tax_amount));
    $('#tp-grand-total').text(fmt(bill.grand_total_amount));

    // Items
    if (Array.isArray(items) && items.length) {
      $('#tp-items-body').html(items.map(i => `
        <tr>
      ...
      $('#tp-items-body').html('<tr><td colspan="5" class="text-center text-muted">No items</td></tr>');
    }
    ...
    try {
      ...
      if (Array.isArray(docs) && docs.length) {
        docs.forEach(f => {
          ...
          const fileArr = JSON.stringify([`../public/uploads/vendor/bill/${f}`]);
          docsHtml += `<div class="preview-card documentclk" data-filetype="documents" data-files='${fileArr}'>`
                    + `<img src="${icon}" style="height:50px;"><div>${f}</div></div>`;
        });
      } else {
        docsHtml = '<span class="text-muted small">No documents</span>';
      }
    } catch(e) { docsHtml = '<span class="text-muted small">No documents</span>'; }
    $('#tp-docs').html(docsHtml);

    $('#tdsDetailPanel').data('bill-id', bill.id);
    openTdsPanel();
  });

  // PDF / Print
  $('#tp-pdf-btn').on('click', function () {
    const id = $('#tdsDetailPanel').data('bill-id');
    $.ajax({ url:'{{ route("superadmin.getpurchasepdf") }}', method:'GET', data:{id}, xhrFields:{responseType:'blob'},
      success: function(data) {
        const link = document.createElement('a'); link.href = URL.createObjectURL(new Blob([data],{type:'application/pdf'}));
        link.download = `tds_bill_${id}.pdf`; document.body.appendChild(link); link.click(); document.body.removeChild(link);
      }
    });
  });
  $('#tp-print-btn').on('click', function () {
    const id = $('#tdsDetailPanel').data('bill-id');
