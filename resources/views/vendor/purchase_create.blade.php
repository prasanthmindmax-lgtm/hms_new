                                ...
                                <div class="search-dropdown">
                                    ...
                                    <div class="dropdown-menu" id="vendor-dropdown">
                                        <div class="search-box">
                                            <input type="text" placeholder="Search" autocomplete="off" autocorrect="off" class="inner-search form-control mb-2">
                                        </div>
              ...
              <div class="card-body">
                  ...
                  <table class="table item-table">
                    ...
                    <tbody>
                      <tr class="item-row">
                        ...
                        <td>
                          <input type="number" name="linesdata[0][quantity]" autocomplete="off" autocorrect="off" class="form-control quantity" value="1.00" min="0" step="0.01"  style="width:100px">
                        </td>
                        <td>
                          <input type="number" name="linesdata[0][rate]" autocomplete="off" autocorrect="off" class="form-control rate" value="0.00" min="0" step="0.01" style="width:100px">
                        </td>
                        <td>
                ...
                <div class="text-start mb-3">
                  ...
                  {{-- <div class="form-check mt-2">
                    <input class="form-check-input" type="checkbox" id="include-po">
                    <label class="form-check-label" for="include-po">Include 1 Open Purchase Orders</label>
                  </div> --}}
                </div>
                ...
                <div class="row d-flex justify-content-between">
                  ...
                  <div class="invoice-totals-box">
                    ...
                    <div class="sub-total-section">
                      <div class="label">Sub Total <span id="taxModeBadge"
                    ...
                    <div class="form-row adjustment-row">
                      ...
                      <input type="number" class="adjustment-value form-control" name="adjustment_value" autocomplete="off" autocorrect="off" placeholder="Enter adjustment">
                ...
                <div class="notes-upload-wrapper">
                  ...
                  <div class="notes-section">
                    ...
                    <textarea id="notes" class="notes-textarea" name="note" autocomplete="off" autocorrect="off" placeholder="Enter your note..."></textarea>
                    <p class="note-hint">It will not be shown in PDF</p>
                  </div>

                  <!-- Upload Section -->
                  <div class="upload-section">
                    <label class="upload-title">Attach File(s) to Purchase</label>
                    ...
                    <div class="mt-2" style="display: flex; gap: 10px;flex-direction:column;">
                       <label>Section<span style="color: red">*</span></label>
                        <div class="tax-dropdown-wrapper tdssection">
                          <input type="text" class="form-control tax-section-input" name="tds_section_name" autocomplete="off" autocorrect="off" placeholder="Select a Tax" readonly>
                          <input type="hidden" name="tds_section_id" class="tds_section_id">
                          <div class="dropdown-menu tax-dropdown">
                            <div class="inner-search-container">
                              <input type="text" class="section-inner-search" placeholder="Search...">
                            </div>
                  ...
                  <form>
                    ...
                    <div class="col-sm-4" style="display: flex; gap: 10px; ">
                      <div style="flex: 1;">
                        ...
                        <select name="gst_type" class="gst_type" autocomplete="off" autocorrect="off" id="gst_type">
...
<script>
      ...
      let taxMode = "exclusive";  // default "Tax Exclusive"
        ...
        $(document).on("click", "#taxModeBadge", function () {
            if (taxMode === "exclusive") {
                taxMode = "inclusive";
                $("#taxModeBadge").removeClass("bg-warning").addClass("bg-success")
                                  .text("TAX INCLUSIVE");
            } else {
                taxMode = "exclusive";
                $("#taxModeBadge").removeClass("bg-success").addClass("bg-warning")
                                  .text("TAX EXCLUSIVE");
            }

            calculateFinalTotals(); // re-calculate
        });

  // function gstcalculate(changedRow) {
  //   // summary object: keys like "CGST_2.50", "SGST_2.50", "IGST_5.00"
  //   const summary = {};

  //   // find all tax-selects on the page and evaluate each row
  //   $('.selected-gst-tax').each(function () {
  //     const $select = $(this);
  //     // attempt to get a sensible row container for this input
  ...
  //     const discountValue = parseFloat($('.discount-percent').val()) || 0;

  //     if (discountType === "percent") {
  //         discountAmount = amount * (discountValue / 100);
  //     } else {
  //         // proportional distribution for fixed amount discount
  //         let totalBeforeDiscount = 0;
  //         $('.selected-gst-tax').each(function () {
  //             const row2 = $(this).closest('tr, .item-row, .invoice-row, .row');
  //             const q = parseFloat(row2.find('.quantity').val()) || 0;
  //             const r = parseFloat(row2.find('.rate').val()) || 0;
  //             totalBeforeDiscount += (q * r);
  //         });

  //         if (totalBeforeDiscount > 0) {
  //             discountAmount = (amount / totalBeforeDiscount) * discountValue;
  //         }
  //     }
  //     if (!isAfterTax) {
  //         amount = amount - discountAmount;   // Discount reduces GST base only in before-tax mode
  //     }
  ...
  //       // save back to row inputs (if present)
  ...
  //       const keyC = `CGST_${Number(cgst_percent).toFixed(2)}`; // normalized key
  //       const keyS = `SGST_${Number(sgst_percent).toFixed(2)}`;
  ...
  //       const keyI = `IGST_${Number(gst_percent).toFixed(2)}`;
  //       summary[keyI] = (summary[keyI] || 0) + gst_amount;
  //     } else {
  //       // unknown type: zero out cgst/sgst
  //       if (row.find('.cgst_amount').length) row.find('.cgst_amount').val('0.00');
  //       if (row.find('.sgst_amount').length) row.find('.sgst_amount').val('0.00');
  //     }
  //   });

  //   // render the aggregated, unique tax list
  //   const $container = $('.gst_calculate_show');
  //   $container.empty();

  //   // order of display: CGST, SGST, IGST (alphabetical-ish)
  //   const orderedKeys = Object.keys(summary).sort((a,b) => {
  //     // ensure CGST before SGST before IGST if same percent
 ...
 <script>
  ...
  $(document).on('input', '.export_amount, .loading_unloading_amount', function() {
    ...
    toastr.options = {
            ...
            // Skip pagination clicks that are inside a TDS/GST modal — those have their own handlers
            if ($(this).closest('.tds-modal').length) return;
            e.preventDefault();
            ...
            $.ajax({
                ...
                success: function(response) {
                    $('.gst-content').html(response); // replace only the table
                },
                error: function() {
                    alert('Something went wrong while loading quotations.');
                }
            });
        });
        ...
        $('#per_page').on('change', function() {
            let url = "{{ route('superadmin.getpurchasecreate') }}"; // update with your actual route
            ...
            //         dataType: 'json',
            //         success: function(response) {
            //             $('#gstModal .gst-modal-body').html(response.html);
            //             $('#gstModal .gst-modal-body').scrollTop(0);
            //         },
            //         error: function(xhr) {
            ...
            $(document).on('click', '#gstModal .pagination a', function(e) {
             ...
             $(document).on('click', '#tdsModal .pagination a', function(e) {
                ...
                $.ajax({
                    ...
                    dataType: 'json',
                    success: function(response) {
                        // Only replace the table content, preserving the "New TDS Tax" header button
            ...
            $('.add_quotation').on('click', function () {
              ...
              $(document).ready(function () {
                ...
                function submitBillForm(saveStatus, $btn) {
                    ...
                    formData.append('taxModeBadge', $('#taxModeBadge').text());
                    ...
                    $.ajax({
                        url: '{{ route("superadmin.savepurchaseorder") }}',
                        method: "POST",
                        data: formData,
                        processData: false,
                        contentType: false,
                        ...
                        success: function (response) {
                            ...
                            setTimeout(() => {
                                ...
                                let redirectUrl =
                                    "{{ route('superadmin.getpurchaseorder') }}" +
                                    "?page=" + redirectPage +
  ...
  @if ($type === 'edit')
    ...
    <script>
      $(document).ready(function () {
                      ...
                      $('.zone-search-input').val(purchase_header[0].zone_name);
                      $('.zone_id').val(purchase_header[0].zone_id);
                      $('.branch-search-input').val(purchase_header[0].branch_name);
                      $('.branch_id').val(purchase_header[0].branch_id);
                      $('.company-search-input').val(purchase_header[0].company_name);
                      $('.company_id').val(purchase_header[0].company_id);
                      $('.delivery-search-input').val(purchase_header[0].delivery_address);
                      $('.delivery_id').val(purchase_header[0].delivery_id);
                      let isFirst = true;
                    ...
                    $('.pf_type').val(purchase_header[0].pf_type);
  ...
  @elseif($type == 'clone')
    ...
    <script>
      $(document).ready(function () {
                      ...
                      $('.delivery-search-input').val(purchase_header[0].delivery_address);
                      $('.delivery_id').val(purchase_header[0].delivery_id);
                      // console.log("purchase_header[0].delivery_id",purchase_header[0].delivery_id);
                      // console.log("purchase_header[0].delivery_address",purchase_header[0].delivery_address);

                      // $('.delivery-search-input,.delivery_name').val(purchase_header[0].delivery_address);
                      let isFirst = true;
