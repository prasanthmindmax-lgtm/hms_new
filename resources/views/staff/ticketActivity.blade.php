            ...
            <div class="card">
              ...
              @if($ticketActivities)
                @foreach ($ticketActivities as $ticketActivity) 
                  @php $createUser = App\Models\usermanagementdetails::
                                join('tbl_ticket_activities','users.id','=', 'tbl_ticket_activities.created_by')
                                ->where('tbl_ticket_activities.created_by', $ticketActivity->created_by)->first();     
                    ...
                    <div class="row">
                      ...
                      <div class="col">
                        <div class="row" >
                          <div class="col">
                            <div class="" >
                              <h4 class="d-inline-block">@if ($ticketActivity->created_by ==  auth()->user()->id ){{ 'You' }} @else {{ $createUser->user_fullname }} @endif</h4>
                        ...
                        @php $ticketImages = App\Models\ImageModel::where('ticket_id',  $ticketActivity->id)->get();    @endphp
                          ...
                          @foreach ($ticketImages as $ticketImage)
                            <div class="col-xl-2 col-lg-3 col-sm-4 col-xs-12">
                              ...
                              @if ($val[1] == 'pdf')
                                <a href="{{ asset('/uploads/'. $ticketImage->imgName) }}" target="_blank" type="application/pdf">
                                  <img src="{{ asset('/assets/images/pdf.png') }}" class="img-fluid" alt="PDF" ></a>
                              @else
                              <div class="vertical-scroll-block grow overflow-x-auto" id="result1">
              ...
              <div class="modal-content">
                ...
                <ul id="save_msgList"></ul>
                  <div class="modal-body">
                      <form method="post" action="{{ route('staff.staffReplyActivity') }}" name="demoform" id="demoform"  enctype="multipart/form-data">
                          @csrf
              ...
              $.ajax({
                ...
                success: function(result) {
                  if(result.status == "success") {
                    ...
                    $("#result1").load(location.href + " #result1");
                    //reset the form
                    $('#demoform')[0].reset();
                    //reset dropzone
