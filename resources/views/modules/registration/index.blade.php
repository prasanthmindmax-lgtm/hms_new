...
<style>
:root{
  --p:#0d9488;--ph:#0f766e;--ps:#ccfbf1;--pk:#5eead4;
  --live:#0ea5e9;--lives:#e0f2fe;
  --local:#8b5cf6;--locals:#ede9fe;
...
/* ── Page Header ─────────────────────────── */
.ph{
  background:linear-gradient(130deg,#064e3b 0%,#0f766e 50%,#0d9488 100%);
  border-radius:16px;padding:22px 26px 18px;color:#fff;
  display:flex;align-items:flex-start;justify-content:space-between;
  flex-wrap:wrap;gap:14px;margin-bottom:18px;
  box-shadow:0 8px 28px rgba(13,148,136,.26);
...
.src-tog button.al{background:rgba(255,255,255,.95);color:#0d9488;}
.src-tog button.lo{background:rgba(255,255,255,.95);color:#8b5cf6;}
...
.sbanner.live {background:var(--lives);color:var(--live);border:1px solid #bae6fd;}
...
.si.t{background:var(--ps);color:var(--p);}
.si.a{background:#fef3c7;color:#b45309;}
.si.i{background:#e0e7ff;color:#4338ca;}
.si.r{background:#ffe4e6;color:#be123c;}
...
.fhchip.date  {background:#f0fdfa;color:#0f766e;border-color:#99f6e4;}
...
.acol{flex:0 0 258px;max-width:258px;}
.tcol{flex:1;min-width:0;}
@media(max-width:768px){.crow{flex-direction:column;}.acol{max-width:100%;}}

/* ── Area Sidebar ────────────────────────── */
.acard{background:var(--card);border:1px solid var(--bdr);border-radius:var(--r);box-shadow:var(--sh);overflow:hidden;}
.achead{
  padding:12px 15px;border-bottom:1px solid var(--bdr);
  font-size:.84rem;font-weight:700;display:flex;align-items:center;justify-content:space-between;
  background:linear-gradient(135deg,#f0fdf4,#f0fdfa);
}
.alist{max-height:520px;overflow-y:auto;padding:12px 14px;}
.aitem{margin-bottom:10px;}
.aitlbl{display:flex;justify-content:space-between;font-size:.73rem;color:var(--txt);margin-bottom:3px;}
.aitlbl strong{color:var(--p);font-weight:700;}
.atrack{background:#e5e7eb;border-radius:20px;height:5px;overflow:hidden;}
.afill{background:linear-gradient(90deg,var(--p),#14b8a6);border-radius:20px;height:100%;transition:width .5s ease;}
...
table.dt tbody tr.clickable:hover{background:#f0fdfa;}
table.dt tbody td{padding:9px 14px;vertical-align:middle;}
.rno{color:var(--sub);font-size:.7rem;font-weight:600;}
.phid{font-family:monospace;font-size:.77rem;background:#f1f5f9;padding:2px 7px;border-radius:5px;color:#4b5563;}
.atag{display:inline-block;font-size:.69rem;padding:2px 9px;border-radius:20px;font-weight:600;background:var(--ps);color:var(--p);}
...
.dmod-hero{
  background:linear-gradient(130deg,#064e3b,#14b8a6);
...
.dmod-hero .sub{font-size:.79rem;opacity:.78;}
.dmod-close{
...
<div class="dmod-bg" id="dmodBg" onclick="closeDmod(event)">
  <div class="dmod" id="dmod">
    <div class="dmod-hero">
      ...
      <div class="sub" id="dm-phid-sub"></div>
    ...
    <div class="dmod-body">
      <div class="dmod-grid">
        ...
        <div class="dmod-field">
          <div class="lbl"><i class="bi bi-calendar-check me-1"></i>Reg. Date</div>
          <div class="val" id="dm-regdate">—</div>
        </div>
        <div class="dmod-field">
          <div class="lbl"><i class="bi bi-gender-ambiguous me-1"></i>Gender</div>
          <div class="val" id="dm-gender">—</div>
        </div>
        <div class="dmod-field">
          <div class="lbl"><i class="bi bi-person me-1"></i>Age</div>
          <div class="val" id="dm-age">—</div>
        </div>
        <div class="dmod-field full">
          <div class="lbl"><i class="bi bi-geo-alt me-1"></i>Area / Location</div>
          <div class="val" id="dm-area">—</div>
        </div>
        <div class="dmod-field full">
          <div class="lbl"><i class="bi bi-fingerprint me-1"></i>Patient ID (PHID)</div>
          <div class="val mono" id="dm-phid-full">—</div>
        </div>
      </div>
    </div>
  </div>
...
<div class="ph">
  <div class="ph-left">
    ...
    <p>New patient registrations from MOC Doc — live or synced local database</p>
  </div>
  <div class="ph-right">
...
<div class="srow" id="statsRow" style="display:none;">
  <div class="sc"><div class="si t"><i class="bi bi-person-check-fill"></i></div>
    <div><div class="sv" id="stTotal">0</div><div class="sl">Total Registrations</div></div></div>
  <div class="sc"><div class="si a"><i class="bi bi-pin-map-fill"></i></div>
    <div><div class="sv" id="stAreas">0</div><div class="sl">Areas Covered</div></div></div>
  <div class="sc"><div class="si i"><i class="bi bi-bar-chart-fill"></i></div>
    <div><div class="sv" id="stTopArea" style="font-size:.88rem;">—</div><div class="sl">Top Area</div></div></div>
  <div class="sc"><div class="si r"><i class="bi bi-calendar-day-fill"></i></div>
    <div><div class="sv" id="stDays">0</div><div class="sl">Days Queried</div></div></div>
</div>
...
<div class="fc">
  ...
  <div class="frow">
    ...
    <div class="fg" id="searchFg" style="display:none;">
      ...
      <input type="text" id="srchInput" placeholder="Name, mobile, PHID…">
    </div>
...
<div class="crow">
  ...
  <!-- Area Sidebar -->
  <div class="acol" id="areaCol" style="display:none;">
    <div class="acard">
      <div class="achead">
        <span><i class="bi bi-bar-chart-line me-1"></i>By Area</span>
        <small id="areaTotalLbl" style="font-size:.71rem;color:var(--sub);"></small>
      </div>
      <div class="alist" id="areaList"></div>
    </div>
  </div>
  ...
  <div class="tcol">
    <div class="tc">
      <div class="ttbar">
        <div>
          <h6 id="tblTitle">Registration Records</h6>
      ...
      <div style="overflow-x:auto;">
        <table class="dt">
          <thead>
            <tr>
              <th>#</th><th>PHID</th><th>Name</th><th>Mobile</th>
              <th>Gender</th><th>Age</th><th>Area</th><th>Reg. Date</th>
            </tr>
          </thead>
          <tbody id="tbody">
            <tr><td colspan="8">
...
function msToggle(id, val, label, itemEl){
  ...
  else            { set.add(val);    if(cb) cb.checked=true;  itemEl.style.background='#f0fdfa'; }
  _updateMsLabel(id);
...
function msSelectAll(id){
  document.querySelectorAll('#list-'+id.replace('ms-','')+' .ms-item').forEach(function(el){
    ...
    if(cb && el.style.display!=='none'){ _ms[id].add(cb.value); cb.checked=true; el.style.background='#f0fdfa'; }
  });
  _updateMsLabel(id);
  if(id==='ms-zone') _loadBranches();
}
...
function _updateMsLabel(id){
  var set=_ms[id], ph={'ms-zone':'All Zones','ms-branch':'All Branches'};
  ...
  if(set.size<=3){
    set.forEach(function(v){
      ...
      $tags.append('<span class="ms-tag">'+_xe(lbl)+'<span class="rm" onclick="event.stopPropagation();msRemove(\''+id+'\',\''+v.replace(/\\/g,'\\\\').replace(/'/g,"\\'")+'\')">×</span></span>');
    });
  } else {
    $tags.html('<span class="ms-label">'+set.size+' selected</span>');
  }
}
function _xe(s){ return s?String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'):''; }
...
_R = {
  fetch:    '{{ route("registration.fetch") }}',
  branches: '{{ route("registration.branches") }}',
  lastSync: '{{ route("registration.last-sync") }}',
};
var R=_R;
...
function doFetch(){
  ...
  showLdr('Fetching registration data…');
  $.get(R.fetch,{
    source:     state.source,
    date_range: dr,
  ...
  .done(function(r){
    ...
    renderTable(r); renderStats(r); renderAreaSidebar(r.area_summary||{}); renderPagination(r); renderFilterBar();
...
function renderTable(r){
  var $b=$('#tbody').empty();
  $('#tblTitle').text('Registrations — All');
  $('#tblSub').html(r.total.toLocaleString()+' unique registrations &nbsp;<span class="'
    +(r.source==='live'?'src-live"><i class="bi bi-wifi"></i> Live':'src-local"><i class="bi bi-database"></i> Local')
    +'</span>');
  if(!r.data.length){
    $b.html('<tr><td colspan="8"><div class="es"><i class="bi bi-inbox"></i><p>No registrations found.</p></div></td></tr>'); return;
  }
  var off=(state.page-1)*state.perPage;
  r.data.forEach(function(row,i){
    var d=row.reg_date?row.reg_date.replace(/(\d{4})(\d{2})(\d{2})/,'$3-$2-$1'):'—';
    var $tr=$('<tr class="clickable">'
      +'<td><span class="rno">'+(off+i+1)+'</span></td>'
      +'<td><span class="phid">'+_xe(row.phid)+'</span></td>'
      +'<td><strong style="font-size:.83rem;">'+(_xe(row.name)||'—')+'</strong></td>'
      +'<td style="font-size:.8rem;">'+(_xe(row.mobile)||'—')+'</td>'
      +'<td style="font-size:.78rem;">'+(_xe(row.gender)||'—')+'</td>'
      +'<td style="font-size:.78rem;">'+(_xe(row.age)||'—')+'</td>'
      +'<td><span class="atag">'+_xe(row.area)+'</span></td>'
      +'<td style="font-size:.78rem;color:var(--sub);">'+d+'</td>'
      +'</tr>');
    (function(rd,df){ $tr.on('click',function(){ openDmod(rd,df); }); })(row,d);
    $b.append($tr);
  });
...
function renderStats(r){
  var sum=r.area_summary||{}, areas=Object.keys(sum);
  $('#stTotal').text(r.total.toLocaleString());
  $('#stAreas').text(areas.length);
  var top=areas[0]||'—';
  $('#stTopArea').text(top.length>14?top.slice(0,12)+'…':top);
...
/* Area Sidebar */
function renderAreaSidebar(summary){
  var entries=Object.entries(summary);
  if(!entries.length){ $('#areaCol').hide(); return; }
  var max=entries[0][1];
  var $list=$('#areaList').empty();
  $('#areaTotalLbl').text(entries.length+(entries.length===1?' area':' areas'));
  entries.slice(0,30).forEach(function(e){
    var area=e[0], cnt=e[1], pct=Math.round(cnt/max*100);
    $list.append('<div class="aitem">'
      +'<div class="aitlbl"><span>'+_xe(area)+'</span><strong>'+cnt+'</strong></div>'
...
$('#btnExport').on('click',function(){
  ...
  $.get(R.fetch,{
    ...
    var hdr=['#','PHID','Name','Mobile','Gender','Age','Area','Reg. Date'];
    dlCSV([hdr].concat(r.data.map(function(row,i){
      return [i+1,row.phid,row.name,row.mobile,row.gender,row.age,row.area,row.reg_date];
    })),'registration_report');
  });
});
...
$('#btnReset').on('click',function(){
  ...
  $('#srchInput').val('');
  $('#tbody').html('<tr><td colspan="8"><div class="es"><i class="bi bi-funnel"></i><p>Select filters and click <strong>Fetch</strong> to load data</p></div></td></tr>');
  $('#tblTitle').text('Registration Records'); $('#tblSub').text('');
  $('#statsRow').hide(); $('#pgw').hide(); $('#btnExport').hide();
  $('#qsearch').hide().val(''); $('#areaCol').hide(); $('#bannerCnt').text('');
  $('#fhbar').hide().empty();
});
...
function renderFilterBar(){
  ...
  chips.forEach(function(c){
    var $chip=$('<span class="fhchip '+c.cls+'"><i class="bi '+c.icon+' me-1"></i>'+_xe(c.label)
...
/* Detail Modal */
function openDmod(row, regDateFmt){
  $('#dm-name').text(row.name||'Unknown Patient');
  $('#dm-phid-sub').html('<i class="bi bi-fingerprint me-1"></i>PHID: '+_xe(row.phid||'—'));
  $('#dm-badges').html(
    '<span class="dbadge"><i class="bi bi-geo-alt me-1"></i>'+_xe(row.area||'—')+'</span>'
    +(row.gender?'<span class="dbadge"><i class="bi bi-person me-1"></i>'+_xe(row.gender)+'</span>':'')
    +(row.age?'<span class="dbadge"><i class="bi bi-calendar me-1"></i>'+_xe(row.age)+' yrs</span>':'')
  ...
  $('#dm-regdate').text(regDateFmt||row.reg_date||'—');
  $('#dm-gender').text(row.gender||'—');
  $('#dm-age').text(row.age ? row.age+' years' : '—');
  $('#dm-area').text(row.area||'—');
  $('#dm-phid-full').text(row.phid||'—');
...
function showErr(m){ $('#tbody').html('<tr><td colspan="8"><div class="es text-danger"><i class="bi bi-exclamation-triangle"></i><p>'+m+'</p></div></td></tr>'); }
