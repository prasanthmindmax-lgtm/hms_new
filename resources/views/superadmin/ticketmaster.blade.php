...
"><i class="ti ti-plus f-18"></i> Add Ticket Manager</a></div>
        ...
        <div class="card-body border-bottom pb-0">
              ...
              <div class="tab-content" id="myTabContent">
                ...
                @php $user_name = App\Models\usermanagementdetails::select('users.id','users.user_fullname')
...
<div class="col-sm-12">
                ...
                <div
                    ...
                    <div class="modal-content">
                        ...
                        <h5 class="modal-title" id="exampleModalLabel1" style="color: #ffffff;font-size: 12px;"> Add Ticket Manager</h5>
                        ...
                        <div id="error-message"></div>
                            ...
                            <div class="modal-body">
							 <form method="post" action="{{ route('superadmin.ticketMasterAdded') }}" name="demoform" id="demoform"  enctype="multipart/form-data">
							@csrf
                                 <input type="hidden" class="ticketmasterId" name="ticketmasterId" id="ticketmasterId" value="">
                                ...
                                <div class="row">
                                    <div class="col-sm-4">
                                        <div class="mb-3">
											...
											<div class="loct-dropdown">
												<input type="text" class="searchLocation loction_name" placeholder="Select Location" style="height: 36px;border-radius: 10px;border: solid 1px #d3d3d3;background-color: #fff;color: #505050 !important;width: 100%;padding-left: 6px;font-size:12px" required name="location">
												<div class="loct-dropdown-options">
													@if($locations)
														@foreach ($locations as $location)
															<div data-value="{{ $location->id }}">{{ $location->name}}</div>
                                    ...
                                    <div class="col-sm-4">
                                        <div class="mb-3">
                                            <label class="form-label required" style="font-size: 12px;font-weight: 600;">Department:</label>&nbsp;&nbsp;<span style="font-size:10px; color:red;"  class="error_department errorss"></span>
											<div class="loct-dropdown">
												<input type="text" class="searchLocation dept_name" placeholder="Select From Department" style="height: 36px;border-radius: 10px;border: solid 1px #d3d3d3;background-color: #fff;color: #505050 !important;width: 100%;padding-left: 6px;font-size:12px" required name="from_department">
												<div class="loct-dropdown-options">
											@if($categories)
												@foreach ($categories as $category)
													<div data-value="{{ $category->id }}">{{ $category->depart_name}}</div>
                                    ...
                                    <div class="col-sm-4">
                                        <div class="mb-3">
                                            <label class="form-label required" style="font-size: 12px;font-weight: 600;">Admin:</label>&nbsp;&nbsp;<span style="font-size:10px; color:red;"  class="error_department errorss"></span>
											<div class="loct-dropdown">
												<input type="text" class="searchLocation user_name" placeholder="Select From Department" style="height: 36px;border-radius: 10px;border: solid 1px #d3d3d3;background-color: #fff;color: #505050 !important;width: 100%;padding-left: 6px;font-size:12px" required name="user_id">
												<div class="loct-dropdown-options">
											@if($user_name)
												@foreach ($user_name as $user)
													<div data-value="{{$user->id}}">{{$user->user_fullname}}</div>
													@endforeach
                                            @endif
												</div>
											</div>
...
<script src="{{ asset('/assets/superadmin/ticket-master.js') }}"></script>
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.status === "success") {
                window.location.href = "{{ route('superadmin.getTicketMaster') }}";
            } else {
...
$(document).on('click', '.edit-btn', function () {
    ...
    // Fill modal input fields with row data
    $('.loction_name').val(tr.data('branch_name'));
    $('.dept_name').val(tr.data('depart_name'));
    $('.user_name').val(tr.data('user_fullname'));

    // (optional) Set hidden fields
    $('.ticketmasterId').val(tr.data('id'));
    $('.userid').val(tr.data('user_id'));
});
</script>
...
<script type="text/javascript">
    ...
    const myticketmasterfetchUrl = "{{ route('superadmin.myTicketMasterFetch') }}";
    const myticketmasterdate= "{{ route('superadmin.myticketmasterdate') }}";
	...
	const fetchMymasterfitter = "{{ route('superadmin.fetchmyticketmasterfitter') }}";
