...
"><i class="ti ti-plus f-18"></i> Add Ticket</a></div>
        ...
        <div class="card-body border-bottom pb-0">
              ...
              <div class="tab-content" id="myTabContent">
                <div class="tab-pane fade show" id="analytics-tab-2-pane" role="tabpanel" aria-labelledby="analytics-tab-2" tabindex="0">
        ...
        <table class="tbl">
            <thead class="thd">
                <tr class="trview">
                    ...
                    <th class="thview">Document</th>
                    <th class="thview" style="width:13%">Status</th>
                    <th class="thview" style="width:10%">Chat</th>
                </tr>
            </thead>
...
<div class="col-sm-12">
                        ...
                        <div id="error-message"></div>
							...
							@csrf
                                ...
                                <div class="row">
                                    ...
                                    <label for="">Upload Attachments (Images and Pdf only)</label></br>
                                    <input name="files[]" id="image_uploads" type="file" multiple accept=".jpg,.jpeg,.png,.gif,.pdf" />
            ...
            <div class="modal-dialog modal-lg modal-dialog-centered">
                ...
                <div class="modal-body" id="fileViewerBody">
                    <!-- Files will load here dynamically -->
                </div>
                </div>
            </div>
            </div>
...
<script>
    // Laravel variable containing all statuses
var statusList = @json($statustable);
var access_limits = @json(auth()->user()->access_limits);

// Helper to map status colors
function getStatusColor(status) {
    switch(status) {
        case "Open": return "#6b75a8";
        case "In-progress": return "#ecca51";
        case "Completed": return "#28a745";           // green
        case "Waiting for Approval": return "#bd478c";
        case "Approved": return "#5cb85c";            // green
        case "Rejected": return "#ee2f2f";
        case "Re-Open": return "#f0ad4e";             // orange
        case "Holded": return "#6c757d";              // gray
        default: return "#aaa";                        // fallback
    }
}
$(document).on('click', '.status-badge', function (e) {
    ...
    $('.status-options').remove(); // remove any open ones

    let $badge = $(this);
    let badgeOffset = $badge.offset();
    let badgeHeight = $badge.outerHeight();

    // Create a new dropdown dynamically (outside the table)
    let $dropdown = $('<div class="status-options"></div>').css({
        position: 'absolute',
    ...
    statusList.forEach(status => {
        if (access_limits === 1) {
            ...
            if (
                ...
                status.status_name !== "Re-Open" &&
                status.status_name !== "Holded"
            ) {
                $dropdown.append(`
                    <div class="status-option-item"
                        data-ticketid="${$badge.data('id')}"
    ...
    // Append dropdown to body (so it's outside the table)
...
// Handle status click
$(document).on('click', '.status-option-item', function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        let badge = $(`.status-badge[data-id='${ticketId}']`);
                        badge.text(newStatus);
                        badge.css("background", getStatusColor(newStatus));
...
$(document).on('click', '.chat-btn', function(e) {
    ...
    const ticketNo = $(this).data('ticket_no');
    const user = $(this).data('user');

    openChatPopup(ticketId, ticketNo, user);
});

function openChatPopup(ticketId, ticketNo, user) {
    // Remove any existing chat popup
    ...
    let chatHtml = `
        <div id="chatPopup" class="chat-popup">
            <div class="chat-header">
                <h5>Ticket #${ticketNo} - ${user}</h5>
                <span class="close-chat" style="cursor:pointer;">&times;</span>
            </div>
            <div class="chat-body" id="chatBody_${ticketId}">
                <p><em>Chat started...</em></p>
            </div>
            <div class="chat-footer">
                <label for="fileUpload_${ticketId}" title="Attach File"><i class="fa fa-paperclip"></i></label>
                <input type="file" id="fileUpload_${ticketId}" class="fileUpload" data-ticket_id="${ticketId}">
                <input type="text" id="chatInput_${ticketId}" class="chat-input" placeholder="Type a message...">
                <button class="send-chat" data-ticket_id="${ticketId}">Send</button>
            </div>
        </div>
    `;
...
function loadChatHistory(ticketId) {
    $.ajax({
        url: `{{ url('/superadmin/ticket/chat/history') }}/${ticketId}`,
        type: 'GET',
        success: function(messages) {
            const chatBody = $(`#chatBody_${ticketId}`);
            chatBody.empty();

            if (messages.length === 0) {
                chatBody.append('<p><em>No messages yet.</em></p>');
                return;
            }
            ...
            messages.forEach(msg => {
                ...
                const side = msg.sender === 'you' ? 'you' : 'other';
                const time = new Date(msg.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
                chatBody.append(`
                    <div class="message ${side}">
                        ${msg.file_url ? `<a href="${msg.file_url}" target="_blank">📎 File</a><br>` : ''}
                        ${msg.message || ''}
                        <div class="time"><span>${msg.user_name} &nbsp; &nbsp;</span>${time}</div>
                    </div>
                `);
            });
...
$(document).on('click', '.send-chat', function() {
    const ticketId = $(this).data('ticket_id');
    const message = $(`#chatInput_${ticketId}`).val().trim();
    const chatBody = $(`#chatBody_${ticketId}`);
...
$(document).on('change', '.fileUpload', function() {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            chatBody.append(`
                <div class="message you">
                    <a href="${response.file_url}" target="_blank">📎 ${file.name}</a>
                    <div class="time">${time}</div>
                </div>
            `);
            chatBody.scrollTop(chatBody.prop("scrollHeight"));
...
$("form[name='demoform']").submit(function(event) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            let formElement = document.getElementById('demoform');
            ...
            $.ajax({
                ...
                success: function(result) {
                    if (result.status === "success") {
                        ...
                        Swal.fire({
                            title: 'Error!',
                            text: result.errors || 'Something went wrong.',
                            icon: 'error'
                        });
                    }
                },
...
<script type="text/javascript">
	...
	const fetchticketfitter = "{{ route('superadmin.fetchticketfitter') }}";
	...
	const fetchAlldepartment = "{{ route('superadmin.department') }}";
...
<script>
        ...
        <script>
            window.baseUrl = "{{ asset('ticket_attachments') }}/";
            console.log("Laravel Base URL:", window.baseUrl);
        </script>
