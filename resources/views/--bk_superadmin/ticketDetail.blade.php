      ...
      <div class="row" id="refresh">
        <div class="col-lg-12">
            <div class="card">
              ...
              @php $createUser = App\Models\HrmUsers::where('id',  $ticketActivity->staff_id)->first();    @endphp
                  ...
                  <div class="row">
                    ...
                    <div class="col">
                      <div class="row">
                        <div class="col">
                          <div class="">
                            <h4 class="d-inline-block">@if ($ticketActivity->staff_id ==  auth()->user()->id ){{ 'You' }} @else {{ $createUser->name }} @endif</h4>
                          ...
                          @if ($ticketImages)
                            @foreach ($ticketImages as $ticketImage)
                              <div class="col-xl-2 col-lg-3 col-sm-4 col-xs-12 button thumbwrap">
                                  ...
                                  @if ($val[1] == 'pdf')
                                    <a href="{{ asset('/public/uploads/'. $ticketImage->imgName) }}" target="_blank" type="application/pdf"><img src="{{ asset('/assets/images/pdf.png') }}" class="img-fluid" alt="PDF" ></a>
                                  @else
                                  <div class="horizontal-scroll-block1 grow overflow-x-auto ">
                                    <div class="flex gap-2 item-start flex-nowrap">
                                      <a class="group block overflow-hidden mb-0 min-w-[115px]" data-fslightbox="gallery" href="{{ asset('/public/uploads/'. $ticketImage->imgName) }}">
                                        <img src="{{ asset('/public/uploads/'. $ticketImage->imgName) }}" class="card-img relative z-10 transition-all duration-300 group-hover:scale-[1.2]" alt="img">
                                      </a>
                                    </div>
                                  </div>
                                  @endif
            ...
            <div class="modal-content">
              ...
              <ul id="save_msgList"></ul>
                <div class="modal-body">
                    <form method="post" action="{{ route('superadmin.superAdminReplyTicket') }}" name="demoform" id="demoform"  enctype="multipart/form-data">
                    @csrf
                        <input type="hidden" class="userid" name="userid" id="userid" value="">
                        <div class="mb-3">
                            <label for="recipient-name" class="col-form-label pt-0">Ticket Status</label>
                        ...
                        <div class="mb-3">
                            <label for="recipient-name" class="col-form-label pt-0">Description</label>
                        ...
                        <input type="hidden" name="priotity_level" id="priotity_level" value="{{ $ticketDetail->priotity_level }}">
...
<script>
        ...
        let category = $(this).val();
            $.ajax({
                method: 'post',
                url: "/testtask/getSuperAdminSubCategory",
                data: {
                    category: category
                },
      ...
      $(document).on('click', '.approve_ticket', function (e) {
          ...
          $.ajax({
              type: "POST",
              url: "/testtask/adminApproveTicket/" + id,
              dataType: "json",
