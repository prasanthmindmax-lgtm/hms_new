...
$("#my_vehicle_search").on("click", function () {
	...
	$(document).on('click', '.dept-dropdown-options div', function () {
		var selectedValue = $(this).data('value');  // Get the value of the selected option       
		
			$('#consultantDiv').hide();
			 $('#bioWasteDiv').hide();
        // Show the respective div based on the selected value
        if (selectedValue == 1) {
            $('#consultantDiv').show();  // Show the "Visiting Consultant" div
        } else if (selectedValue == 2) {
            $('#bioWasteDiv').show();  // Show the "Bio Waste" div
        }
	});
