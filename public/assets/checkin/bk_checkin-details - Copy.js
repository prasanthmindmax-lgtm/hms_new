	...
	var data = Array.isArray(response?.data) ? response.data : response;
		...
		$.each(pageData, function(index, usr) {				
				...
				body += '<tr onclick="rowClick(event)">' +
					  ...
					  '<td class="tdview" id="amount_view" data-billdate="' + usr.billdate + '" data-mrd="' + usr.phid + '" data-billno="' + usr.bill_no + '" data-billtype="' + usr.billtype + '"  data-paymenttype="' + usr.paymenttype + '" style="cursor: pointer;color: rgb(16 35 255);">' + usr.bill_no + '</td>' +
		...
		$.each(pageData, function(index, usr) {
				...
				body += '<tr onclick="rowClick(event)">' +
					 ...
					 /* '<td class="tdview"><a href="' + fullURL + '">' + usr.phid + '</a></td>'+*/
					  '<td class="tdview" id="" data-id="' + usr.id + '">AFERD-6651</td>' +
					    '<td class="tdview">Tamilselvi</td>' +
					    '<td class="tdview">25-05-2025</td>' +
	...
	$(document).on('click', '.clickable-date', function (e) {
		...
		$.ajax({
                ...
                success: function(response) {
					...
					handleSuccess(response,2);
					handleSuccess(response,3);
                }
            });
	});
