...
$(document).on("input keyup", ".editable span", function () {
    ...
    if (cls.includes("cash_radiant")) {
        ...
        tr.find(".cash_diff span").text((rad - moc).toFixed(2));
    }
    ...
    if (cls.includes("card_radiant")) {
        let moc = toNum(cell.find(".tooltip-text").text());
        let rad = toNum(cell.find(".edit-text").text());
        tr.find(".card_diff span").text((rad - moc).toFixed(2));
    }
    ...
    if (cls.includes("upi_radiant")) {
        let moc = toNum(cell.find(".tooltip-text").text());
        let rad = toNum(cell.find(".edit-text").text());
        tr.find(".upi_diff span").text((rad - moc).toFixed(2));
    }
    ...
    if (cls.includes("card_radiant") ||
        ...
        let difference = bank_stmt_amt - expected;
    ...
    if (cls.includes("neft_bank")) {
        ...
        tr.find(".neft_diff span").text((bank - moc).toFixed(2));
    }
});
...
//                         td.find(".file-view")
//                           .removeClass("d-none")
//                           .attr("href", window.location.origin + "/hms_new/public/radiant_files/" + filePath);
...
//     alert("Values applied! Now click SAVE to store into DB.");
// });
$(document).on("click", ".save-btn", function () {
    ...
    if (cashRadiant === "") return alert("Cash Radiant Required");

    let cardRadiant = tr.find(".card_radiant span").text().trim();
    if (cardRadiant === "") return alert("Card Orange Required");
    ...
    let data = {
        ...
        cash_mocdoc: tr.find(".cash_mocdoc").text().trim(),
        cash_radiant : tr.find(".cash_radiant .edit-text").text(),
        cash_radiant_diff : tr.find(".cash_diff .edit-text").text(),
        ...
        cash_bank_diff    : tr.find(".cash_bank_diff span").text(),
        card_mocdoc: tr.find(".card_mocdoc").text().trim(),
        card_radiant : tr.find(".card_radiant .edit-text").text(),
        card_radiant_diff : tr.find(".card_diff .edit-text").text(),
        card_date_filter : tr.find(".card_radiant .calander-text").text(),
        card_date_amt_filter : tr.find(".card_radiant .tooltip-text").text(),
        upi_mocdoc: tr.find(".upi_mocdoc").text().trim(),
        upi_radiant  : tr.find(".upi_radiant .edit-text").text(),
        upi_radiant_diff  : tr.find(".upi_diff .edit-text").text(),
        ...
        neft_mocdoc : tr.find(".neft_mocdoc").text(),
        neft_bank : tr.find(".neft_bank .edit-text").text(),
        neft_bank_diff : tr.find(".neft_diff span").text(),
    ...
    $.ajax({
        ...
        success:function(res){
            ...
            /* ==========================================================
               COMMON FUNCTION — SAME AS loadRadiantValues()
            ========================================================== */
            function updateBlock(cls, radiant, date, amt, remark, file, column){
                ...
                td.append(`<div class="custom-tooltip">Loading...</div>`);
                ...
                if(date && date !== "-"){
                    ...
                    if(column !== ''){
                        ...
                        $.ajax({
                            ...
                            success:function(res){
                                buildTooltip(td, res, date, remark, file);
                            },
...
function loadRadiantValues(tr) {
    ...
    $.ajax({
        ...
        success: function(res) {
            ...
            // -------- FILE UPLOAD HANDLING (from applyRadiantFiles) --------
            const fileMap = {
                cash_radiant: 'cash_radiant_file',
                cash_bank: 'cash_bank_file',
                card_radiant: 'card_radiant_file',
            ...
            Object.keys(fileMap).forEach(field => {
                ...
                if (filePath) {
                    ...
                    // Reset if no file
                    td.find(".plus-icon").show();
                    td.find(".file-view").addClass("d-none").attr("href", "#");
                }
            });
