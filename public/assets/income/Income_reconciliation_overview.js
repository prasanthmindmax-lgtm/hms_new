...
function ticketrenderTable(data, ticketpageSize, pageNum) {
    ...
    if (Actual) {
        $('#actuall_cash').text(parseFloat(Actual.cash || 0).toFixed(2));
        $('#actuall_card').text(parseFloat(Actual.card || 0).toFixed(2));
        $('#actuall_neft').text(parseFloat(Actual.neft || 0).toFixed(2));
        $('#actuall_upi').text(parseFloat(Actual.upi || 0).toFixed(2));
        $('#actuall_amount').text(parseFloat(Actual.total || 0).toFixed(2));
    } else {
        // Reset if no consolidated row found
        $('#actuall_cash').text('0.00');
        $('#actuall_card').text('0.00');
        $('#actuall_neft').text('0.00');
        $('#actuall_upi').text('0.00');
        $('#actuall_amount').text('0.00');
    }
...
$("#daily_details_recon tr").each(function () {
    console.log("loadRadiantValues call");
...
function ticketData(pageData, body) {
    ...
    pageData.forEach(usr => {
        ...
        const cashMocAmt = parseFloat(usr.cash_moc_amt) || 0;
        const cashRadiant = parseFloat(usr.cash_radiant) || 0;
        const cashBank = parseFloat(usr.cash_bank) || 0;
        const cardMocAmt = parseFloat(usr.card_moc_amt) || 0;
        const cardRadiant = parseFloat(usr.card_radiant) || 0;
        const upiMocAmt = parseFloat(usr.upi_moc_amt) || 0;
        const upiRadiant = parseFloat(usr.upi_radiant) || 0;
        const bankStmtCharge = parseFloat(usr.bank_stmt_charge) || 0;
        const bankStmtAmount = parseFloat(usr.bank_stmt_amount) || 0;
        const neftMocAmt = parseFloat(usr.neft_moc_amt) || 0;
        const neftBank = parseFloat(usr.neft_bank) || 0;

        // Calculate differences
        const cashDiff = parseFloat(usr.cash_radiant_diff) || 0;
        const cashBankDiff = parseFloat(usr.cash_bank_diff) || 0;
        const cardDiff = parseFloat(usr.card_radiant_diff) || 0;
        const upiDiff = parseFloat(usr.upi_radiant_diff) || 0;
        const bankStmtDiff = parseFloat(usr.bank_stmt_diff) || 0;
        const neftDiff = parseFloat(usr.neft_bank_diff) || 0;

        // Add to totals
        totals.cash_moc_amt += cashMocAmt;
        totals.cash_radiant += cashRadiant;
        totals.cash_bank += cashBank;
        totals.card_moc_amt += cardMocAmt;
        totals.card_radiant += cardRadiant;
        totals.upi_moc_amt += upiMocAmt;
        totals.upi_radiant += upiRadiant;
        totals.bank_stmt_charge += bankStmtCharge;
        totals.bank_stmt_amount += bankStmtAmount;
        totals.neft_moc_amt += neftMocAmt;
        totals.neft_bank += neftBank;
        totals.cash_diff += cashDiff;
        totals.cash_bank_diff += cashBankDiff;
        totals.card_diff += cardDiff;
        totals.upi_diff += upiDiff;
        totals.bank_stmt_diff += bankStmtDiff;
        totals.neft_diff += neftDiff;

        body += `
        <tr>
            <td class="tdview">#${sno}</td>
            <td class="tdview date-cell" data-date="${usr.date_range}">${usr.date_range}</td>
            <td class="tdview zone-cell">${usr.zone_name}</td>
            ...
            <td class="tdview cash_diff"><span>${cashDiff.toFixed(2)}</span></td>
            ...
            <td class="tdview cash_bank_diff"><span>${cashBankDiff.toFixed(2)}</span></td>
            ...
            <td class="tdview card_diff"><span>${cardDiff.toFixed(2)}</span></td>
            ...
            <td class="tdview upi_diff" ><span>${upiDiff.toFixed(2)}</span></td>
            ...
            <td class="tdview bank_stmt_diff"><span>${bankStmtDiff.toFixed(2)}</span></td>
            ...
            <td class="tdview neft_diff" ><span>${neftDiff.toFixed(2)}</span></td>
    ...
    <tr class="total-row" style="background-color: #f8f9fa; font-weight: bold;">
        <td class="tdview" colspan="4">Total</td>
        <td class="tdview">${totals.cash_moc_amt.toFixed(2)}</td>
        <td class="tdview">${totals.cash_radiant.toFixed(2)}</td>
        <td class="tdview">${totals.cash_diff.toFixed(2)}</td>
        <td class="tdview">${totals.cash_bank.toFixed(2)}</td>
        <td class="tdview">${totals.cash_bank_diff.toFixed(2)}</td>
        <td class="tdview">${totals.card_moc_amt.toFixed(2)}</td>
        <td class="tdview">${totals.card_radiant.toFixed(2)}</td>
        <td class="tdview">${totals.card_diff.toFixed(2)}</td>
        <td class="tdview">${totals.upi_moc_amt.toFixed(2)}</td>
        <td class="tdview">${totals.upi_radiant.toFixed(2)}</td>
        <td class="tdview">${totals.upi_diff.toFixed(2)}</td>
        <td class="tdview">${totals.bank_stmt_charge.toFixed(2)}</td>
        <td class="tdview">${totals.bank_stmt_amount.toFixed(2)}</td>
        <td class="tdview">${totals.bank_stmt_diff.toFixed(2)}</td>
        <td class="tdview">${totals.neft_moc_amt.toFixed(2)}</td>
        <td class="tdview">${totals.neft_bank.toFixed(2)}</td>
        <td class="tdview">${totals.neft_diff.toFixed(2)}</td>
    </tr>
    `;
...
function calculateRow(tr) {
    ...
    setVal(tr.find(".cash_diff"), tr.find(".cash_diff span"), cash_rad - cash_moc);
    setVal(tr.find(".cash_bank_diff"), tr.find(".cash_bank_diff span"),cash_bank - cash_rad );
    ...
    setVal(tr.find(".card_diff"), tr.find(".card_diff span"),card_rad_change -  card_moc);
    setVal(tr.find(".card_bank_diff"), tr.find(".card_bank_diff span"), card_rad_change - card_bank);
    ...
    setVal(tr.find(".upi_diff"), tr.find(".upi_diff span"), upi_rad_change - upi_moc);
    setVal(tr.find(".upi_bank_diff"), tr.find(".upi_bank_diff span"),upi_rad_change - upi_bank);
    ...
    setVal(tr.find(".neft_diff"), tr.find(".neft_diff span"),neft_bank - neft_moc);
    ...
    let expected = (card_rad + upi_rad) - charge;
    let difference =bank_amt - expected;
