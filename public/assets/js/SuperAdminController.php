...
// dd($access_limit);
    switch ($access_limit) {
        ...
        case 2: 
       $doctordetails->where(function($query) use ($user) {
    ...
    ->orWhere('ref_doctor_details.empolyee_name', $user->id);  // Include the logged-in user themselves
});
    ...
    foreach ($fitterremovedata as $condition) {
        ...
        switch ($column) {
            ...
            case 'userfullname':
                ...
                if ($user) $userFilter = $user_name->id;
                break;
        ...
        $endDateFormatted = now()->toDateString(); 
       ...
       case 3: 
        ...
        //    dd($zoneFilter,$branchFilter,$userFilter);
        if ($zoneFilter) {
    ...
    public function fetchmorefitterremove(Request $request)
{
    ...
    // Process fitterremovedataall if it exists
    if ($fitterremovedataall) {
        ...
        foreach (explode(' AND ', $fitterremovedataall) as $condition) {
            ...
            switch ($column) {
                ...
                default:
                    // Handle other columns with comma-separated values
...
public function leadsfilter(Request $request)
{
    ...
    try {
        ...
        $data = CampLeadsModel::select(
            ...
            ->join('tblzones', 'tbl_locations.zone_id', '=', 'tblzones.id')
            ->whereBetween('hms_camp_leads.created_at', [$startDate, $endDate]);
    ...
    public function expensesalldetails()
{
    ...
    public function menuaccessurl()
    {
        ...
        $menus = DB::table('users as u')
            ->leftJoin('user_menus as um', 'u.id', '=', 'um.user_id')
            ->leftJoin('menus as m', 'm.id', '=', DB::raw('COALESCE(um.menu_id)'))
            ->where('um.user_id', $userids)
            ->distinct()
            ->select('m.menu_name', 'm.route', 'm.icon', 'u.user_fullname', 'u.role_id', 'm.active_ids')
            ->get();
