$(document).ready(function () {
    ...
    $('#submit-neft-datas').click(function (event) {
        ...
        formData.append('approval_status', $('#approval_status').val());
        
        // AJAX Request
        $.ajax({
            url: purchaseapproversaveUrl, // Update with your actual endpoint
            type: "POST",
            data: formData,
...
function renderTable(data, pageSize, pageNum) {
    ...
    $.each(pageData, function(index, user) {
        ...
        body += '<tr class="tdview" onclick="rowClick(event)" style="cursor:pointer;" data-id="' + user.id + '">' +
                ...
                '<i class="ti ti-plus f-18"></i>' +
                '</button></td>' +
                '<td class="tdview" style="text-align: center; vertical-align: middle;">' +'<button class="btn btn-sm btn-success edit-btn">' +' Approval ' +'</button>' +'</td>';
            body += '</tr>';
