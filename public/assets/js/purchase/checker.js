$(document).ready(function () {
    ...
    $('#close_button').click(function () {
     ...
     $('#submit-neft-datas').click(function (event) {
        ...
        formData.append('checker_status', $('#checker_status').val());
        ...
        body += '<tr class="tdview" onclick="rowClick(event)" style="cursor:pointer;" data-id="' + user.id + '">' +
                ...
                '<td class="tdview" id="checker_status1" data-checker_status="' + (user.approval_status ?? '-') + '">' +
                    (user.approval_status == 0 
                        ...
                        : '<button class="btn btn-sm btn-success">Success</button>') +
                '</td>' +
                // '<td class="tdview" id="approval_status1" data-approval_status="' + (user.approval_status ?? '-') +'">' + (user.approval_status ==0 ? 'Pending': 'Success') + '</td>' +
                ...
                '<td style="display:none;" id="po_delivery_upload1" class="tdview" data-filetype="po_delivery" data-files=\'' + JSON.stringify(user.po_delivery_upload) + '\'></td>' +
                ...
                '<i class="ti ti-plus f-18"></i>' +
                '</button></td>' +
                '<td class="tdview" style="text-align: center; vertical-align: middle;">' +'<button class="btn btn-sm btn-success edit-btn">' +' Check' +'</button>' +'</td>';
            body += '</tr>';
            body += '<tr class="docs-row" id="docs-row-' + user.id + '" style="display: none;">' +
                    '<td colspan="25">' +
                    '<div class="docs-content" id="docs-content-' + user.id + '"></div>' +
                    '</td>' +
                    '</tr>';
              });
...
$(document).on('click', '.edit-btn', function () {
    ...
    // Fill form fields
    (id !== "") ? $("#id").val(id).attr('readonly', true) : $("#id").val('').removeAttr('readonly');
    (user_id !== "") ? $("#users_id").val(user_id).attr('readonly', true) : $("#users_id").val('').removeAttr('readonly');
    //  $("#users_id").val(user_id);
    (serial_number !== "") ? $("#serial_number").val(serial_number).attr('readonly', true) : $("#serial_number").val('').removeAttr('readonly');
    // $("#serial_number").val(serial_number);
    (created_by !== "") ? $("#created_by").val(created_by).attr('readonly', true) : $("#created_by").val('').removeAttr('readonly');
    // $("#created_by").val(created_by);
    (vendor !== "") ? $("#vendor").val(vendor).attr('readonly', true) : $("#vendor").val('').removeAttr('readonly');
    // $("#vendor").val(vendor);
    (nature_payment !== "") ? $("#nature_payment").val(nature_payment).attr('readonly', true) : $("#nature_payment").val('').removeAttr('readonly');
    // $("#nature_payment").val(nature_payment);
    (invoice_amount !== "") ? $("#invoice_amount").val(invoice_amount).attr('readonly', true) : $("#invoice_amount").val('').removeAttr('readonly');
    // $("#invoice_amount").val(invoice_amount);
    (already_paid !== 0 &&  already_paid !== "") ?  $("#already_paid").val(already_paid).attr('readonly', true) : $("#already_paid").val('').removeAttr('readonly');
    // $("#already_paid").val(already_paid);
    (pan_number !== "") ? $("#pan_number").val(pan_number).attr('readonly', true) : $("#pan_number").val('').removeAttr('readonly');
    // $("#pan_number").val(pan_number);
    (account_number !== "" && account_number !==null) ? $("#account_number").val(account_number).attr('readonly', true) : $("#account_number").val('').removeAttr('readonly');
    // $("#account_number").val(account_number);
    (ifsc_code !== "") ? $("#ifsc_code").val(ifsc_code).attr('readonly', true) : $("#ifsc_code").val('').removeAttr('readonly');
    // $("#ifsc_code").val(ifsc_code);
    (payment_status !== "") ? $("#payment_status").val(payment_status).attr('readonly', true) : $("#payment_status").val('').removeAttr('readonly');
    // $("#payment_status").val(payment_status);
    (utr_number !== "") ? $("#utr_number").val(utr_number).attr('readonly', true) : $("#utr_number").val('').removeAttr('readonly');
