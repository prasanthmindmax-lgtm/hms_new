...
$('.ranges, .applyBtn').on('click', function() {
    var isPending = $('.pending_overview').is(':visible');
    var isSaved = $('.saved_overview').is(':visible');
    var moredatefittervale, datefiltersave;
    if ($(this).hasClass('ranges')) {
        moredatefittervale = getDateRangeForApi('#mydateviewsall');
        datefiltersave = getDateRangeForApi('#mydateviewsallsave');
        ...
        if (dateRange.length >= 2) {
            ...
            moredatefittervale = convertDateFormat(dateRange[0]) + ' - ' + convertDateFormat(dateRange[1]);
            datefiltersave = moredatefittervale;
        } else {
            moredatefittervale = getDateRangeForApi('#mydateviewsall');
            datefiltersave = getDateRangeForApi('#mydateviewsallsave');
        }
    }
    var fitter = typeof fitterremovedata !== 'undefined' ? fitterremovedata : [];
    if (isPending && moredatefittervale) ticketdatefillterrange(moredatefittervale, fitter);
    else if (isSaved && datefiltersave) datefillterrange(datefiltersave, fitter);
});
...
function cancelformdata() {
      startLoader();
		...
		$.ajax({
			...
			success: function (responseData) {
                stopLoader(true);
				handleSuccessCancel(responseData);
			},
  ...
  function cancelsavefilterview(uniqueResults,url,moredatefittervale, ph_id = '', status_filter = '') {
          startLoader2();
            $("#sveddata_tbl").hide();
...
function handleSuccessCancel(responseData) {
    $("#document_tbl").show();
...
var labelToStatKey = { 'Total Raised': 'total_raised', 'Admin Approved': 'admin_approved', 'Zonal Approved': 'zonal_approved', 'Audit Approved': 'audit_approved', 'Final Approved': 'final_approved', 'Pending': 'pending', 'Total Discount': 'total_discount_amount', 'Total Cancelled': 'total_discount_amount' };
...
function renderStatisticsCards(statistics, counts) {
    ...
    if (typeof applyStatFilter === 'function') {
        $container.find('.stat-card').off('click.stats').on('click.stats', function () {
            var filter = $(this).data('stat-key') || $(this).data('filter');
            applyStatFilter((filter || '').toLowerCase().replace(/_/g, '-'));
...
function renderTablesaved(data, pageSizedocuments, pageNum, isApprover) {
    ...
    function renderStatus(status, approverName) {
        ...
        let icon = '';
        if (status === 1) {
            icon = `<span style="color:green;font-weight:bold;font-size:20px;">✔</span>`;
        } else if (status === 2) {
            icon = `<span style="color:red;font-weight:bold;font-size:20px;">✖</span>`;
        } else {
            icon = `<span style="color:#f0ad4e;font-weight:bold;font-size:14px;">⏳ Pending</span>`;
        }
    ...
    $.each(pageData, function (index, user) {
        ...
        // ===============================
        // CHECKBOX (pending rows only - same as discount)
        // ===============================
        let isPending = (access_limits == 1 && user.final_approver == 0) ||
            (access_limits == 2 && user.zonal_approver == 0) ||
            (access_limits == 3 && user.admin_approver == 0) ||
            (access_limits == 4 && user.audit_approver == 0);
        let checkboxCell = '';
        ...
        if (access_limits == 1) {
            body += `
                ...
                <td class="tdview" data-column="dit-approver">${renderStatus(user.audit_approver, user.audit_approver_name)}</td>
...
function patchCancelSavedTableRow(rec) {
    ...
    if (!moredatefittervale || moredatefittervale === 'Today' || moredatefittervale.indexOf('-') === -1) {
        moredatefittervale = safeDate;
    }
 ...
 var fitterremovedata = [];
    ...
    var safeDate = getDateRangeForApi('#mydateallviewssave');
    if (!datefiltersave || datefiltersave === 'Today' || datefiltersave.indexOf('-') === -1) {
        datefiltersave = safeDate;
    }
   ...
   $(document).on('click', '.savedata_options div', function () {
    ...
    $(".value_save_mysearch").each(function (index) {
     ...
     fitterremovedata = resultsArray_marketer.map(filter => filter.replace(/, /g, ','));
    // Call function with the processed data
    cancelsavefilterview(fitterremovedata,cancelformsave_data,moredatefittervale, '', $('#save_status_filter').val());
});
...
$('#save_mrd_views').on('input', function () {
    ...
    fitterremovedata = resultsArray_marketer.map(filter => filter.replace(/, /g, ','));
    cancelsavefilterview(fitterremovedata,cancelformsave_data,moredatefittervale, ph_id, $('#save_status_filter').val());
});

$('#save_status_filter').on('change', function () {
    var moredatefittervale = getDateRangeForApi('#mydateallviewssave');
    var ph_id = ($('#save_mrd_views').val() || '').trim();
    var statusVal = $(this).val();
    if (fitterremovedata && fitterremovedata.length) {
        cancelsavefilterview(fitterremovedata, cancelformsave_data, moredatefittervale, ph_id, statusVal);
    } else {
        cancelsaveformdata();
    }
});
...
function updateTableRow($row, record, status) {
    ...
    let $auditCell = $row.find('[data-column="dit-approver"]');
...
function updateCountBadges(counts) {
    ...
    var amt = counts.total_discount_amount;
    $('.total-discount-amount').text(amt !== undefined ? '₹' + (parseFloat(amt) || 0).toFixed(2) : '₹0');
}
