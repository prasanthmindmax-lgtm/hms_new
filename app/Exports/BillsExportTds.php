...
/**
 * Export all bill data + TDS section (tax_amount, section name, tds_pay_date, tds_challan_no, tds_quarter, tds_paid_status, etc.)
 * Same filters as BillsExport / getbill. Supports xlsx and csv.
 */
class BillsExportTds implements FromCollection, WithHeadings, WithEvents
{
    protected $request;
    protected $format;
    ...
    public function collection()
    {
        ...
        return $bills->flatMap(function ($bill) {
            ...
            return $bill->BillLines->map(function ($line) use ($bill, $tdsSectionName, $tdsTaxName, $tdsTaxRate) {
                return [
                    $bill->bill_date,
                    $bill->due_date,
                    $bill->zone_name,
