...
public function incomeupdatedetails(Request $request)
{
    ...
    public function regularAudit() 
    {
        $staffId = auth()->user()->id;
        $admin = auth()->user();
        return view('superadmin.regular_audit', compact('admin'));
    }    
...
private function zoneMapping(){
    ...
    private function CCName(){
     ...
     public function regularFetch(Request $request){
            set_time_limit(0); 
            $array = [];            
            return response()->json($array);             
    }

     public function saveRegularAudit(Request $request) 
    {
        echo "<pre>";print_r($request->all());exit;
        $data = [
                    ...
                    'package_paid' => $request->package_paid,
                    'ip' => $request->ip,
                    'days_of_injection' => $request->days_of_injection,
                    'injection_used' => $request->injection_used,
                    'value' => $request->value,
                    'mention_if_done_verified' => $request->mention_if_done_verified,
                    'date_of_freezing' => $request->date_of_freezing,
                    'straw_detach' => $request->straw_detach,
                    'cost_of_paid' => $request->cost_of_paid,
                    'split_up' => $request->split_up,
                    'paid_details' => $request->paid_details,
                    'd_o_r' => $request->d_o_r,
                    'fet_paid_details' => $request->fet_paid_details,
                    'legal_charges' => $request->legal_charges,
                    'spl_media_charges' => $request->spl_media_charges,
                    'husband_name' => $request->husband_name,
                    'husband_age' => $request->husband_age,
                    'comprehensive' => $request->comprehensive,
                    'legal_fees' => $request->legal_fees,
                    'D5_blast' => $request->D5_blast,
                    'macs' => $request->macs,
                    'microflidises' => $request->microflidises,
                    'ds' => $request->ds,
                    'tesa' => $request->tesa,
                    'seman_freezing' => $request->seman_freezing,
                    'scopy_details' => $request->scopy_details,
                    'remarks' => $request->remarks,
                    'created_at' => now(),
                ];
        TblRegularAudit::insert($data);
         return redirect()->back()
            ->with('success', 'Regular audit inserted successfully!'); 
    } 
    ...
    public function checkInTimeLine (Request $request){
        ...
        $url = 'https://mocdoc.com/api/get/patienttimeline/draravinds-ivf/AFCPK-3408'; 
...
public function incomereportAPI($filterdate, $fitterremovedataall, $apistatus, $statusid, $url, $indates)
{
    ...
    foreach ($indates as $dte) {
        if (strpos($fitterremovedataall, 'tblzones.name') !== false && strpos($fitterremovedataall, 'tbl_locations.name') === false) {
            ...
            // echo "<pre>333111113";print_r($zone_location);exit; 
            foreach ($zone_location as $zone) {
                ...
                if (!empty($income_data['billinglist'])) {
                    foreach ($income_data['billinglist'] as $billing) {
                        $inc_Data[] = $this->saveCurlData($billing, $zlocation, $apistatus, $zone->name,$zone_name->name);                       
                        }                       
                    }   
                } 				
                $status_id = 1;
        ...
        elseif (strpos($fitterremovedataall, 'tblzones.name') !== false && strpos($fitterremovedataall, 'tbl_locations.name') !== false) {
            ...
            // echo "<pre>";print_r($dat_values);exit;                                          
...
private function incomeBranchDateFiltr($fitterremovedataall,$moredatefittervale,$apistatus,$statusid){
    ...
    // echo "<pre>11111111ee";print_r($incomeArray);exit; 
