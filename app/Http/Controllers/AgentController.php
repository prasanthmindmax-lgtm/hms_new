...
class AgentController extends Controller
{
    ...
    public function storeLoanImage(Request $request)
    {
        $files = [];
        $ticketId = $request->userid ?? '';
        if ($request->hasFile('file')) {
            ...
            foreach ($files as $key => $file) {
                LoanDocModel::updateOrCreate(
                    ['loan_id'   => '0000'],['doc_image' => $file['name'],'loan_id' => $ticketId]
                );
            }
        }
        $id = LoanDetails::find($ticketId);
        return response()->json(['status'=>"success",'userid'=>$ticketId]);
    }
    ...
    public function getTicket()
    {
        ...
        return view('staff.ticket', compact('admin','priorities','locations','statuses','categories'));
    }
    ...
    public function ticketActivity($id)
    {
        ...
        $admin = auth()->user();
        return view('staff.ticketActivity', compact('ticketDetail', 'admin', 'ticketActivities'));
    }
