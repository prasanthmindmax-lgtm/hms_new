    ...
    public function savevendor(Request $request)
{
    ...
    // Check if the requested vendor_id already exists
    $exists = Tblvendor::where('vendor_id', $vendor_id)->exists();

    if ($exists) {
        // Get the last vendor ID in the system
        ...
        if ($lastVendor && preg_match('/VEN-(\d+)/', $lastVendor->vendor_id, $matches)) {
            ...
            $vendor_id = 'VEN-' . $nextNumber;
        } else {
            // Fallback if no valid vendor_id found
            $vendor_id = $request->vendor_id;
        }
    }
    $data = [
    ...
    if ($request->filled('universal_search')) {
        ...
        $detailsQuery->where(function ($q) use ($search) {
            $q->where('bill_gen_number', 'like', "%{$search}%")
              ...
              ->orWhereHas('BillLines', function ($b) use ($search) {
                  $b->where('description', 'like', "%{$search}%");
              });
        });
    }
