...
class SuperAdminController extends Controller
{
  ...
  public function referral()
    ...
    // {
    //      $admin = auth()->user();
    //     $camps = Campmanagement::get();
    //     return view('superadmin.camps', ['admin' => $admin, 'camps' => $camps]);
    ...
    public function camp()
    {
        ...
        // return view('superadmin.camp', ['admin' => $admin]);
        // dd($admin);
        return view('superadmin.campses', ['admin' => $admin]);
    }

       public function campdashboard()
    {
         $admin = auth()->user();
        $camps = Campmanagement::get();
        // return view('superadmin.camps', ['admin' => $admin, 'camps' => $camps]);
        // return view('superadmin.camp', ['admin' => $admin]);
        // dd($admin);
        return view('superadmin.campdashboard', ['admin' => $admin]);
    }

     public function addcampActivities(Request $request, $id)
    {
        $admin = auth()->user();
        $campids = Campmanagement::where('id', $id)->first();
        // $campids = $request->id;
        return view('superadmin.addcampActivities', ['admin' => $admin, 'campids' => $campids]);
    }
     ...
     public function addnewcamp()
    {
        $admin = auth()->user();
        $camps = Campmanagement::get();
        return view('superadmin.addnewcamp', ['admin' => $admin, 'camps' => $camps]);
    }

    public function addcampleads(Request $request, $id)
    {
        $admin = auth()->user();
        $campids = Campmanagement::where('id', $id)->first();
        return view('superadmin.addcamplead', ['admin' => $admin, 'campids' => $campids]);
    }
    public function addnewcampleads(Request $request)
    {
        $admin = auth()->user();
        return view('superadmin.addnewcampleads', ['admin' => $admin]);
    }

    public function campleads()
    {
        $admin = auth()->user();
        $camps = Campmanagement::get();
        // return view('superadmin.camps', ['admin' => $admin, 'camps' => $camps]);
        return view('superadmin.campleads', ['admin' => $admin]);
    }
    public function campactivities()
    {
        $admin = auth()->user();
        $camps = Campmanagement::get();
        // return view('superadmin.camps', ['admin' => $admin, 'camps' => $camps]);
        return view('superadmin.campactivities', ['admin' => $admin]);
    }
    public function addnewcampActivities()
    {
        $admin = auth()->user();
        $camps = Campmanagement::get();
        // return view('superadmin.camps', ['admin' => $admin, 'camps' => $camps]);
        return view('superadmin.addnewcampActivities', ['admin' => $admin]);
    }
    ...
    //     $id = $request->id;
    //     // dd($id);
    //     $camp_manage = Campmanagement::where('id', $id)->orderBy('id','desc')->first();
    //     // $camp_lead = CampLeadsModel::where('camp_zone_id', $id)->orderBy('camp_id','desc')->get();
    //     $camp_lead = CampLeadsModel::where('camp_main_id', $id)->orderBy('camp_id','desc')->get();

    //     $camp_activity = CampActivityModel::where('camp_main_id', $id)->orderBy('campa_id','asc')->get();
    //     // dd($camp_activity);
    //     // $camp_activity = CampActivityModel::where('campa_zone_id', $id)->orderBy('campa_id','asc')->get();

    //     return view('superadmin.campAjax',compact('camp_manage','camp_lead','camp_activity'));
    // // return response()->json([
    // //     'camp_lead' => $camp_lead,
    // //     'camp_activity' => $camp_activity
    // // ]);
    // }
      ...
      public function campaign_details(Request $request)
    {
        ...
        return view('superadmin.campAjax',compact('camp_manage','camp_lead','camp_activity','id'));
    }
    ...
    public function doctoradded(Request $request)
    {
        $validatedData = $request->validate([
           ...
           'hospital_link' => 'required|string|max:2000',
            'map_link' => 'required|string|max:2000',
        ]);
        $imagePaths = [];

         $user_fullname_record = DB::table('users')
                            ->where('username', $request->empolyee_name)
                            ->select('user_fullname')
                            ->first();

    if (!$user_fullname_record) {
        return response()->json(['success' => false, 'message' => 'User not found.']);
    }

    $user_fullname = $user_fullname_record->user_fullname;
    ...
    //     } else if ($access_heads == $access_heads && $access_limit == 0){
    //         $doctordetails = doctordetails::where('access_heads', $access_heads)
...
public function fetchfitter(Request $request)
{
 ...
 if ($zonalheadFilter) {
    $query->where(function($query) use ($zonalheadFilter, $zonalheadname) {
        $query->whereIn('ref_doctor_details.empolyee_name', function($subQuery) use ($zonalheadFilter) {
                $subQuery->select('username')
                    ->from('users')
                    ->where('reporting_manager', $zonalheadFilter);
            })
            ->orWhere('ref_doctor_details.empolyee_name', $zonalheadname);
    });
}
      ...
      public function fetchmorefitterdateclr(Request $request)
{
    ...
    public function meetinginsert(Request $request)
    {
        ...
        foreach ($meetinginsertvalue as $item) {
         ...
         if (isset($data['meeting_image']) && !empty($data['meeting_image'])) {
        $filename = basename($data['meeting_image']);
         $data['meeting_image'] = $filename;
      } else {
        $data['meeting_image'] = null;
    }
 ...
 public function uploadMeetingImage(Request $request)
{
    if ($request->hasFile('meeting_image')) {
        // dd(1);
        $file = $request->file('meeting_image');
        $filename = $file->getClientOriginalName();
        $file->move(public_path('meeting_images'), $filename);
...
// public function meetingallviews(Request $request)
// {
//     $user = auth()->user();
//     $userid = $user->id;

//     // Get access_limit from access_log table instead of users table
//     $accessLog = DB::table('access_log')
//                  ->where('employee_id', $userid)
//                  ->first();
...
//     $access_limit = $accessLog->access_limits;
//     $access_heads = $user->access_heads;  // Still from users table as per original
//     $ref_doctor_id = $request->input('ref_doctor_id');

//     $doctordetails = DB::table('ref_meeting_log')
//         ->join('ref_doctor_details', 'ref_meeting_log.ref_doctor_id', '=', 'ref_doctor_details.id')
//         ->join('branches', 'ref_doctor_details.city', '=', 'branches.id')
//         ->join('users', 'users.username', '=', 'ref_doctor_details.empolyee_name')
//         ->select('ref_meeting_log.*');

//     if($ref_doctor_id != ""){
//         $doctordetails->where('ref_meeting_log.ref_doctor_id', $ref_doctor_id);
//     }
...
//         case 2:
//             $doctordetails->join('tbl_locations', 'ref_doctor_details.city', '=', 'tbl_locations.id')
//                           ->where('tbl_locations.zone_id', $accessLog->zone_id)
//                           ->where(function($query) use ($user) {
//                               $query->where('users.reporting_manager', $user->id)
//                                     ->orWhere('ref_meeting_log.empolyee_name', $user->id);
//                           });
//             break;

//         case 3:
//             $doctordetails->where('ref_meeting_log.empolyee_name', $userid);
//             break;
...
//     $results = $doctordetails->orderBy('ref_meeting_log.id', 'desc')->get();
...
public function meetingallviews(Request $request)
{
    ...
    $doctordetails = DB::table('ref_meeting_log')
        ->join('ref_doctor_details', 'ref_meeting_log.ref_doctor_id', '=', 'ref_doctor_details.id')
        ->join('tbl_locations', 'ref_meeting_log.city', '=', 'tbl_locations.id')
        ->join('users', 'users.username', '=', 'ref_meeting_log.empolyee_name')
        ->select('ref_meeting_log.*');
    ...
    switch ($access_limit) {
        ...
        case 2: // Zonal Head
            $doctordetails->where('tbl_locations.zone_id', $accessLog->zone_id)
                    ->where(function ($query) use ($user) {
                        $query->whereIn('ref_meeting_log.empolyee_name', function ($subQuery) use ($user) {
                            $subQuery->select('username')
                            ->from('users')
                            ->where('reporting_manager', $user->id);                    })
                        ->orWhere('ref_meeting_log.empolyee_name', $user->username);                });
                break;


        case 3:
            $doctordetails->where('ref_meeting_log.empolyee_name', $username);
            break;
...
public function meetingmorefitter(Request $request)
{
    ...
    foreach ($fitterremovedata as $condition) {
        ...
        switch ($column) {
            ...
            case 'userfullname':
                $user = DB::table('users')->where('user_fullname', $value)->first();
    ...
    $query = meetingdetails::query()
        ->join('tbl_locations', 'ref_meeting_log.city', '=', 'tbl_locations.id')
        ->join('tblzones', 'tbl_locations.zone_id', '=', 'tblzones.id')
        ->leftJoin('users', 'ref_meeting_log.empolyee_name', '=', 'users.username')
        ->whereBetween('ref_meeting_log.created_at', [$startdates, $enddates])
        ->select('tbl_locations.name as Branch_name', 'tblzones.name as zone_name', 'ref_meeting_log.*', 'users.user_fullname');
    ...
    switch ($access_limit) {
        ...
        case 2: // Zonal head - only their zone
            $query->where(function($query) use ($user) {
                $query->whereIn('ref_meeting_log.empolyee_name', function($subQuery) use ($user) {
                    $subQuery->select('username')
                        ->from('users')
        ...
        case 3: // Marketer - only their own records
            $query->where('ref_meeting_log.empolyee_name', $user->username);
            break;
    ...
    // Log::debug($formattedQuery); // Uncomment if you want to log the query
...
public function meetingclrfitter(Request $request)
{
    ...
    public function patientinsert(Request $request)
    {
        ...
        // Optional: Return a success response
        return response()->json(['message' => 'patient data inserted successfully']);
    }
...
//     ->join('ref_doctor_details', 'ref_patient_details.ref_doctor_id', '=', 'ref_doctor_details.id')
//     ->join('branches', 'ref_doctor_details.city', '=', 'branches.id')
//     ->join('users', 'users.username', '=', 'ref_doctor_details.empolyee_name')
//     ->select('ref_patient_details.*');


//     if($ref_doctor_id != ""){
//         $doctordetails->where('ref_patient_details.ref_doctor_id', $ref_doctor_id);
//     }
...
//                           ->where(function($query) use ($user) {
//                               $query->where('users.reporting_manager', $user->id)
//                                     ->orWhere('ref_patient_details.empolyee_name', $user->id);
//                           });
//             break;

//         case 3:
//             $doctordetails->where('ref_patient_details.empolyee_name', $userid);
//             break;
...
//     // if ($access_limit != 1 && $access_heads) {
//     //     $doctordetails->where('ref_doctor_details.access_heads', $access_heads);
//     // }

//     $results = $doctordetails->orderBy('ref_patient_details.id', 'desc')->get();
...
public function patientallviews(Request $request)
{
    ...
    $doctordetails = DB::table('ref_patient_details')
        // ->join('ref_doctor_details', 'ref_patient_details.ref_doctor_id', '=', 'ref_doctor_details.id')
        ->join('tbl_locations', 'ref_patient_details.city', '=', 'tbl_locations.id')
        ->join('users', 'users.username', '=', 'ref_patient_details.empolyee_name')
        ->select('ref_patient_details.*');

        // dd($doctordetails->get());
    ...
    switch ($access_limit) {
        ...
        case 3:
            $doctordetails->where('ref_patient_details.empolyee_name', $username);
            break;
    ...
    public function patientmorefitter(Request $request)
{
    ...
    $query = patientdetails::query()
        ->join('tbl_locations', 'ref_patient_details.city', '=', 'tbl_locations.id')
        ->join('tblzones', 'tbl_locations.zone_id', '=', 'tblzones.id')
        ->leftJoin('users', 'ref_patient_details.empolyee_name', '=', 'users.username')
        ->whereBetween('ref_patient_details.created_at', [$startdates, $enddates])
        ->select('tbl_locations.name as Branch_name', 'tblzones.name as zone_name', 'ref_patient_details.*', 'users.user_fullname');
    ...
    switch ($access_limit) {
        ...
        case 2: // Zonal head - only their zone
            $query->where(function($query) use ($user) {
                $query->whereIn('ref_patient_details.empolyee_name', function($subQuery) use ($user) {
                    $subQuery->select('username')
                        ->from('users')
                        ->where('reporting_manager', $user->id);
                })
                ->orWhere('ref_patient_details.empolyee_name', $user->username);
            });
            break;

        case 3: // Marketer - only their own records
            $query->where('ref_patient_details.empolyee_name', $user->username);
            break;
...
//         $find = TblLocationModel::find($Branch);
//         //  dd($find);
//         // Handle image uploads
...
//         $user = auth()->user();
//         $userid = $user->id;
//         $accessLog = DB::table('access_log')->where('employee_id', $userid)->first();
//         $access_limit = $accessLog->access_limits;

//         // Create camp entry
//         $camp = Campmanagement::create(
//             array_merge(
//                 $validatedData,
//                 ['Branch' => $find->name], // explicitly added if renamed
//                 ['Branch_id' => $Branch], // explicitly added if renamed
//                 ['user_id' => $userid],
...
public function campdetailsadded(Request $request)
{
    ...
    $find = TblLocationModel::find($Branch);
    $find_zone = DB::table('tblzones')->where('id', $find->zone_id)->first();

    // Handle Notices_img uploads
    if ($request->hasFile('Notices_img')) {
        foreach ($request->file('Notices_img') as $image) {
            $filename = uniqid() . '_' . $image->getClientOriginalName();
            $destinationPath = public_path('camp_expenses');
            $image->move($destinationPath, $filename);
            $imagePaths[] = 'camp_expenses/' . $filename;
        }
    }

    // Handle Banner_img uploads
    if ($request->hasFile('Banner_img')) {
        foreach ($request->file('Banner_img') as $image) {
            $filename = uniqid() . '_' . $image->getClientOriginalName();
            $destinationPath = public_path('camp_expenses');
            $image->move($destinationPath, $filename);
            $camp_notices[] = 'camp_expenses/' . $filename;
        }
    }
    ...
    $accessLog = DB::table('access_log')->where('employee_id', $userid)->first();
    $access_limit = $accessLog?->access_limits;
// dd($find_zone->id,$find_zone->name);
    // Create camp entry
    $camp = Campmanagement::create([
        'Camp_Date' => $request->Camp_Date,
        'Camp_enddate' => $request->Camp_enddate,
        'Camp_Centre_Name' => $request->Camp_Centre_Name,
        'Camp_Location' => $request->Camp_Location,
        'Camp_Executives' => $request->Camp_Executives,
        'Digital_Marketing_coordinator' => $request->Digital_Marketing_coordinator,
        'Digital_Marketing_Cost' => $request->Digital_Marketing_Cost,
        'Digi_Days' => $request->Digi_Days,
        'Total_Cost' => $request->Total_Cost,
        'Budget_For_Auto' => $request->Budget_For_Auto,
        'Auto_Cost' => $request->Auto_Cost,
        'Auto_Days' => $request->Auto_Days,
        'Auto_Total_Cost' => $request->Auto_Total_Cost,
        'Budget_For_Snacks' => $request->Budget_For_Snacks,
        'Snacks_Cost' => $request->Snacks_Cost,
        'Notices_Cost' => $request->Notices_Cost,
        'Notices_Count' => $request->Notices_Count,
        'Banner_Cost' => $request->Banner_Cost,
        'Banner_Count' => $request->Banner_Count,
        'Dr_attended' => $request->Dr_attended,
        'Branch' => $find->name,
        'Branch_id' => $Branch,
        'user_id' => $userid,
        'zone_id' => $find_zone->id ?? "null",
        'zone_name' => $find_zone->name ?? "null",
        'po_details' => $request->po_details,
        'status' => 'Scheduled',
        'Notices_img' => json_encode($imagePaths),
        'Banner_img' => json_encode($camp_notices),
    ]);
...
public function campdetailsupdate(Request $request)
{
    $update_id = $request->id;

    // Find the camp by ID
    $camp = Campmanagement::findOrFail($update_id);
    ...
    // Handle Notices image uploads (replace if new files uploaded)
    if ($request->hasFile('Notices_img')) {
        foreach ($request->file('Notices_img') as $image) {
            $filename = uniqid() . '_' . $image->getClientOriginalName();
            $destinationPath = public_path('camp_expenses');
        ...
        $camp->Notices_img = json_encode($imagePaths);
    }

    // Handle Banner image uploads (replace if new files uploaded)
    if ($request->hasFile('Banner_img')) {
        foreach ($request->file('Banner_img') as $image) {
            $filename = uniqid() . '_' . $image->getClientOriginalName();
$destinationPath = public_path('camp_expenses');
            $image->move($destinationPath, $filename);
            $bannerPaths[] = 'camp_expenses/' . $filename;
        }
        $camp->Banner_img = json_encode($bannerPaths);
    }

    // Update other fields directly from request inputs (no validation)
    $camp->Camp_Date = $request->Camp_Date;
    $camp->Camp_enddate = $request->Camp_enddate;
    $camp->Camp_Centre_Name = $request->Camp_Centre_Name;
    $camp->Camp_Location = $request->Camp_Location;
    $camp->Camp_Executives = $request->Camp_Executives;
    $camp->Digital_Marketing_coordinator = $request->Digital_Marketing_coordinator;
    $camp->Digital_Marketing_Cost = $request->Digital_Marketing_Cost;
    $camp->Digi_Days = $request->Digi_Days;
    $camp->Total_Cost = $request->Total_Cost;
    $camp->Budget_For_Auto = $request->Budget_For_Auto;
    $camp->Auto_Cost = $request->Auto_Cost;
    $camp->Auto_Days = $request->Auto_Days;
    $camp->Auto_Total_Cost = $request->Auto_Total_Cost;
    $camp->Budget_For_Snacks = $request->Budget_For_Snacks;
    $camp->Snacks_Cost = $request->Snacks_Cost;
    $camp->Notices_Cost = $request->Notices_Cost;
    $camp->Notices_Count = $request->Notices_Count;
    $camp->Banner_Cost = $request->Banner_Cost;
    $camp->Banner_Count = $request->Banner_Count;
    $camp->status = $request->status;
    $camp->Dr_attended = $request->Dr_attended;

    // Update Branch and zone info
    $camp->Branch = $find->name;
    $camp->Branch_id = $Branch;
    $camp->zone_id = $find_zone?->id;
    $camp->zone_name = $find_zone?->name;
    ...
    return response()->json([
     ...
     public function superadmincampdateanddataftters(Request $request)
    {
        // dd($request->all());
        $fitterremovedata = $request->input('fitterremovedata');
        $moredatefittervale = $request->input('moredatefittervale');
        ...
        // $startDateFormatted = \Carbon\Carbon::createFromFormat('d/m/Y', $startDate)->format('Y-m-d');
        // $endDateFormatted = \Carbon\Carbon::createFromFormat('d/m/Y', $endDate)->format('Y-m-d');
        $startDateFormatted = \Carbon\Carbon::createFromFormat('d/m/Y', $startDate)->format('Y-m-d') . ' 00:00:00';
        $endDateFormatted = \Carbon\Carbon::createFromFormat('d/m/Y', $endDate)->format('Y-m-d') . ' 23:59:59';
        $startdates = $startDateFormatted . ' 00:00:00';
        $enddates = $endDateFormatted . ' 23:59:59';
        // Start the query
        $query = Campmanagement::query()->join('branches', 'Camp_management_system.Branch', '=', 'branches.branch_name')->join('zones', 'branches.zone_id', '=', 'zones.id')->select('Camp_management_system.*', 'branches.branch_name as branchnames', 'zones.zone_name');
    ...
    public function campdateanddataftters(Request $request)
{
    ...
    if (!$accessLog) {
        return response()->json(['error' => 'Access log not found'], 404);
    }
    ...
    $query = Campmanagement::query()
        ->join('tbl_locations', 'camp_management_system.Branch', '=', 'tbl_locations.name')
        ->join('tblzones', 'tbl_locations.zone_id', '=', 'tblzones.id')
        ->select('camp_management_system.*', 'tbl_locations.name as branchnames', 'tblzones.name')
        ->whereBetween('camp_management_system.Camp_Date', [$startDateFormatted, $endDateFormatted]);

    // 🔐 Access Limits Logic
    if ($access_limit == 3) {
        // Regular user – only their own camps
        $query->where('camp_management_system.user_id', $userid);
    } elseif ($access_limit == 2) {
        // Admin – own + subordinates
        $reportingUserIds = DB::table('users')
            ->where('reporting_manager', $userid)
            ->pluck('id')
            ->toArray();
        $allUserIds = array_merge([$userid], $reportingUserIds);
        $query->whereIn('camp_management_system.user_id', $allUserIds);
    }
    // Super Admin – no filter needed

    // 🔍 Filtering based on dynamic filters
    if (is_array($fitterremovedata)) {
        $fitterremovedata = implode(' AND ', $fitterremovedata);
    } elseif (!is_string($fitterremovedata)) {
        return response()->json(['error' => 'Invalid fitterremovedata format'], 400);
    ...
    $columnMap = [
        'zone_name' => 'tblzones.name',
        'branch_name' => 'tbl_locations.name',
...
public function campalldetails()
    {
        ...
        $campQuery = Campmanagement::query();

        if ($access_limit == 3) {
            // 🧍‍♂️ Regular user – only their own camps
            // dd($access_limit);
            $campQuery->where('user_id', $userid);
        } elseif ($access_limit == 2) {
            // 👨‍💼 Admin – own + users they manage

            // Get users who report to this admin
            $reportingUserIds = DB::table('users')
                ->where('reporting_manager', $userid)
                ->pluck('id') // gives you just the IDs
                ->toArray();
            // dd($reportingUserIds);
            ...
            $allUserIds = array_merge([$userid], $reportingUserIds);
            // dd($allUserIds);
            // Get all camps for these users
            $campQuery->whereIn('user_id', $allUserIds);
        } else {
            // 👑 Super Admin – no filter, get everything
            // No need to apply filters
        }

        $campdetails = $campQuery->orderBy('Camp_Date', 'desc')->get();
...
public function campdatefitters(Request $request)
{
    ...
    try {
        ...
        return response()->json(['error' => 'Date format must be d/m/Y - d/m/Y'], 400);
    }
    ...
    // Start base query
    $campQuery = Campmanagement::whereBetween('Camp_Date', [$startDate, $endDate]);
    ...
    if ($user) {
        ...
        if ($accessLog) {
            ...
            if ($access_limit == 3) {
                // Regular user
                $campQuery->where('user_id', $user->id);
            } elseif ($access_limit == 2) {
                // Manager with direct reports
                $reportingUserIds = DB::table('users')->where('reporting_manager', $user->id)->pluck('id')->toArray();
                $userIds = array_merge([$user->id], $reportingUserIds);
                $campQuery->whereIn('user_id', $userIds);
            }
            // If access_limit is 1 (Super Admin), no filter needed
        } else {
            // No access_log? Default to user only
            $campQuery->where('user_id', $user->id);
        }
    }
    // If no authenticated user, return everything (or restrict - up to you)
    ...
    return response()->json($campdetails);
     ...
     public function supercampdatefitters(Request $request)
    {
        // Get date range from request
        $datefilltervalue = $request->input('datefilltervalue');
        $dates = explode(' - ', $datefilltervalue);
        ...
        $endDateview = $dates[1] ?? null;

        if (!$startDate || !$endDateview) {
            return response()->json(['error' => 'Invalid date range'], 400);
        }
        ...
        // $campQuery = Campmanagement::whereBetween('Camp_Date', [$startDateFormatted, $endDateFormatted]);
        $campQuery = Campmanagement::whereBetween('Camp_Date', [$startDateFormatted, $endDateFormatted]);
        ...
        if ($access_limit == 3) {
            ...
            // Admin – their own + reporting users

            $reportingUserIds = DB::table('users')->where('reporting_manager', $userid)->pluck('id')->toArray();
    ...
    public function campGraphDashboard(Request $request)
{
    try {
        $zoneFilter = $request->input('morefilltersall'); // corrected from 'fitterremovedata'
        ...
        if ($dateRange && strpos($dateRange, ' - ') !== false) {
            ...
            try {
                $startDateFormatted = \Carbon\Carbon::createFromFormat('d/m/Y', trim($startDate))->format('Y-m-d') . ' 00:00:00';
                $endDateFormatted = \Carbon\Carbon::createFromFormat('d/m/Y', trim($endDate))->format('Y-m-d') . ' 23:59:59';
            } catch (\Exception $e) {
                return response()->json([
        ...
        $query = Campmanagement::query()
            ...
            ->select(
                ...
                'camp_management_system.id as camp_id',
                'camp_management_system.Camp_Date'
            );
        ...
        // Zone filter
        if (!empty($zoneFilter) && str_contains($zoneFilter, "zone_name=")) {
            $zoneName = trim(str_replace("zone_name=", "", $zoneFilter), "'\"");
        ...
        foreach ($campData->groupBy('zone_id') as $zoneId => $zoneCamps) {
            $zoneName = $zoneCamps->first()->zone_name;
            $campIds = $zoneCamps->pluck('camp_id')->toArray();

            $leadCountQuery = \DB::table('hms_camp_leads')->where('camp_zone_id', $zoneId);

            if ($startDateFormatted && $endDateFormatted) {
                $leadCountQuery->whereBetween('created_at', [$startDateFormatted, $endDateFormatted]);
            }
...
public function campGraphDashboardBranch(Request $request)
{
    try {
        $globalFromDate = $request->input('globalFromDate');
        $globalToDate = $request->input('globalToDate');
        ...
        if ($globalFromDate && $globalToDate) {
            try {
                if (strpos($globalFromDate, '/') !== false) {
                    $startDateFormatted = \Carbon\Carbon::createFromFormat('d/m/Y', trim($globalFromDate))->startOfDay();
                    $endDateFormatted = \Carbon\Carbon::createFromFormat('d/m/Y', trim($globalToDate))->endOfDay();
                } else {
                    $startDateFormatted = \Carbon\Carbon::createFromFormat('Y-m-d', trim($globalFromDate))->startOfDay();
                    $endDateFormatted = \Carbon\Carbon::createFromFormat('Y-m-d', trim($globalToDate))->endOfDay();
                }
            } catch (\Exception $e) {
                return response()->json([
                    'error' => 'Invalid date format',
        ...
        // Branch filter logic
        $branchesQuery = \DB::table('tbl_locations');

        if ($selectedBranchName) {
            // If a specific branch is selected, filter by exact name
            $branchesQuery->where('name', $selectedBranchName);
        } else {
            // If no branch is selected, show only Chennai-related branches
            $branchesQuery->where('name', 'like', '%Chennai%');
        }
        ...
        foreach ($branches as $branch) {
            $campQuery = \DB::table('camp_management_system')
                ->where('Branch_id', $branch->id);

            if ($startDateFormatted && $endDateFormatted) {
                $campQuery->where(function ($q) use ($startDateFormatted, $endDateFormatted) {
                    $q->whereBetween('Camp_Date', [$startDateFormatted, $endDateFormatted])
                        ->orWhereBetween('Camp_enddate', [$startDateFormatted, $endDateFormatted]);
                });
            }

            $camps = $campQuery->get();
            $campIds = $camps->pluck('id')->toArray();
            $campCount = count($campIds);

            $leadQuery = \DB::table('hms_camp_leads')->whereIn('camp_main_id', $campIds);

            if ($startDateFormatted && $endDateFormatted) {
                $leadQuery->whereBetween('created_at', [$startDateFormatted, $endDateFormatted]);
            }

            $leadCount = $leadQuery->count();

            if ($campCount > 0 || $leadCount > 0) {
                $result[] = [
    ...
    public function newactivitedatasave(Request $request)
    {
        $campa_zone_id = $request->input('campa_zone_id');
        $campa_name = $request->input('campa_name');
        $camp_id = $request->input('camp_id');

        $tloc = TblLocationModel::where('name', $campa_zone_id)->first();
        // $tcamp = Campmanagement::where('id', $camp_id)->first();
        $tcamp = Campmanagement::where('Camp_Centre_Name', $campa_name)->first();
        ...
        CampActivityModel::create(
            array_merge($validatedData, [
                'user_id' => $userid,
                'camp_location_id' => $tloc->id,
                'camp_loc_name' => $tloc->name,
                'camp_main_id' => $tcamp->id,
                'campa_login_time' => $loginTime,
                'campa_logout_time' => $logoutTime,
                'campa_zone_id' => $tloc?->zone_id,
        ...
        return response()->json(['success' => true, 'message' => 'Camp Activity saved successfully!']);

        //   return view('superadmin.campactivities')
        //        ->with('success', true)
        //        ->with('message', 'Security saved successfully!');
    }


    public function updateactivitedatasave(Request $request)
{
    $update_id = $request->input('update_id');

    // Get the activity record by custom primary key
    $activity = CampActivityModel::where('campa_id', $update_id)->first();
    ...
    // Get location and camp info
    $zoneName = $request->input('campa_zone_id');
    $campName = $request->input('campa_name');

    $tloc = TblLocationModel::where('name', $zoneName)->first();
    $tcamp = Campmanagement::where('Camp_Centre_Name', $campName)->first();
    ...
    $validatedData = $request->validate([
        ...
        'campa_description' => 'required|string|max:255',
        'campa_zone_id' => 'required|string|max:255',
        'imagesnotes.*' => 'nullable|file|mimes:jpeg,png,jpg,gif,webp|max:12048',
        'imagesbanner.*' => 'nullable|file|mimes:jpeg,png,jpg,gif,webp|max:12048',
    ]);
    ...
    // Only process if new notes images are uploaded
    if ($request->hasFile('imagesnotes')) {
        $newImagePaths = [];
        foreach ($request->file('imagesnotes') as $image) {
            $filename = time() . '_' . $image->getClientOriginalName();
            $destinationPath = public_path('camp_activites');
            $image->move($destinationPath, $filename);
            $newImagePaths[] = 'camp_activites/' . $filename;
        }
    }

    // Only process if new banner images are uploaded
    if ($request->hasFile('imagesbanner')) {
        $newBannerPaths = [];
        foreach ($request->file('imagesbanner') as $image) {
            $filename = time() . '_' . $image->getClientOriginalName();
            $destinationPath = public_path('camp_activites');
            $image->move($destinationPath, $filename);
            $newBannerPaths[] = 'camp_activites/' . $filename;
        }
    }
    ...
    $updateData = array_merge($validatedData, [
        'user_id' => auth()->id(),
        'camp_location_id' => $tloc->id ?? null,
        'camp_loc_name' => $tloc->name ?? null,
        'camp_main_id' => $tcamp->id ?? null,
        'campa_login_time' => $request->input('campa_login_time'),
        'campa_logout_time' => $request->input('campa_logout_time'),
        'campa_zone_id' => $tloc?->zone_id ?? null,
        'created_by' => $username,
        'status' => 1,
    ]);
    ...
    if ($newImagePaths !== null) {
        $updateData['campa_notes_img'] = json_encode($newImagePaths);
    }

    if ($newBannerPaths !== null) {
        $updateData['campa_banner_img'] = json_encode($newBannerPaths);
    }
...
// upadate lead
    ...
    public function updatecampleadsdata(Request $request)
{
    ...
    $validatedData = $request->validate([
        ...
        'camp_name' => 'required|string|max:255', // required for fetching camp info
    ]);

    $update_id = $request->input('update_id');
    $campa_name = $request->input('camp_name');

    // Get existing lead
    $lead = CampLeadsModel::where('camp_id',$update_id)->first();

    // Get camp info based on name
    $tcamp = Campmanagement::where('Camp_Centre_Name', $campa_name)->first();
    if (!$tcamp) {
        return response()->json(['success' => false, 'message' => 'Camp not found.'], 404);
    }

    // Get location info
    $tloc = TblLocationModel::where('id', $tcamp->Branch_id)
        ->orWhere('name', $tcamp->Branch)
        ->first();
    ...
    $lead->update(array_merge($validatedData, [
        ...
        'camp_loc_name' => $tloc->name,
        'camp_main_id' => $tcamp->id,
        'camp_name' => $tcamp->Camp_Centre_Name,
        'camp_zone_id' => $tloc?->zone_id,
        'updated_by' => $username,
    ]));

    return response()->json(['success' => true, 'message' => 'Camp Lead updated successfully!']);
}
...
//         $data = CampActivityModel::select('tbl_locations.name as location_name', 'tblzones.name as zone_name', 'hms_camp_activites.*')
//             ->join('tbl_locations', 'hms_camp_activites.camp_location_id', '=', 'tbl_locations.id')
//             ->join('tblzones', 'tbl_locations.zone_id', '=', 'tblzones.id')
//             // ->whereBetween('hms_camp_activites.created_at', [$startDateFormatted, $endDateFormatted]);
//             ->whereBetween('hms_camp_activites.created_at', [$startDateFormatted, $endDateFormatted]);
...
//         } elseif ($access_limit == 2) {
//             // Admin with team access
//             $reportingUserIds = DB::table('users')->where('reporting_manager', $userid)->pluck('id')->toArray();
//             $allUserIds = array_merge([$userid], $reportingUserIds);
//             $data->whereIn('user_id', $allUserIds);
//         }
//         // access_limit == 1 → Super Admin: no user restriction
...
//         $result = $data->orderBy('hms_camp_activites.created_at', 'desc')->get();
...
public function activitydatafilter(Request $request)
    ...
    public function campleadsnewadddata(Request $request)
    {
        // dd($request->all());
        $username = auth()->user()->user_fullname;
        $validatedData = $request->validate([
        ...
        $camp_id = $request->input('camp_id');
        //  $campa_zone_id = $request->input('camp_zone_id');
        $campa_name = $request->input('camp_name');
        //  dd($campa_name);
        //  $campa_name = $request->input('camp_name');
        ...
        // $tcamp = Campmanagement::where('id', $camp_id)->first();
        $tcamp = Campmanagement::where('Camp_Centre_Name', $campa_name)->first();
        $tloc = TblLocationModel::where('id', $tcamp->Branch_id)->orwhere('name', $tcamp->Branch)->first();
        // dd($tloc);
        $user = auth()->user();
        $userid = $user->id;

        CampLeadsModel::create(
            array_merge($validatedData, [
                'user_id' => $userid,
                'camp_location_id' => $tloc->id,
                'camp_loc_name' => $tloc->name,
...
//         // ✅ Build base query
//         $data = CampLeadsModel::select('tbl_locations.name as location_name', 'tblzones.name as zone_name', 'hms_camp_leads.*')
//             ->join('tbl_locations', 'hms_camp_leads.camp_zone_id', '=', 'tbl_locations.id')
//             ->join('tblzones', 'tbl_locations.zone_id', '=', 'tblzones.id')
//             ->whereBetween('hms_camp_leads.created_at', [$startDateFormatted, $endDateFormatted]);
...
//             $data->where('user_id', $userid);
//         } elseif ($access_limit == 2) {
//             // Admin - include reporting users
//             $reportingUserIds = DB::table('users')->where('reporting_manager', $userid)->pluck('id')->toArray();
//             $allUserIds = array_merge([$userid], $reportingUserIds);
//             $data->whereIn('user_id', $allUserIds);
//         }
//         // access_limit == 1 => Super Admin – no restriction
...
//         // 📦 Return data sorted
//         $results = $data->orderBy('hms_camp_leads.created_at', 'desc')->get();
...
//         // ✅ Build base query
//         $data = CampLeadsModel::select('tbl_locations.name as location_name', 'tblzones.name as zone_name', 'hms_camp_leads.*')
//             ->leftjoin('tbl_locations', 'hms_camp_leads.camp_zone_id', '=', 'tbl_locations.id')
//             ->leftjoin('tblzones', 'tbl_locations.zone_id', '=', 'tblzones.id')
//             ->whereBetween('hms_camp_leads.created_at', [$startDateFormatted, $endDateFormatted]);
...
//         if ($access_limit == 3) {
//             // Regular User
//             $data->where('hms_camp_leads.user_id', $userid);
//         } elseif ($access_limit == 2) {
//             // Admin - include reporting users
//             $reportingUserIds = DB::table('users')->where('reporting_manager', $userid)->pluck('id')->toArray();
//             $allUserIds = array_merge([$userid], $reportingUserIds);
//             $data->whereIn('hms_camp_leads.user_id', $allUserIds);
//         }
//         // access_limit == 1 => Super Admin – no restriction
...
//     }
        public function leadsfilter(Request $request)
    {
        ...
        // ✅ Build base query
        $data = CampLeadsModel::select('tbl_locations.name as location_name', 'tblzones.name as zone_name', 'hms_camp_leads.*')
            ->leftjoin('tbl_locations', 'hms_camp_leads.camp_location_id', '=', 'tbl_locations.id')
            ->leftjoin('tblzones', 'tbl_locations.zone_id', '=', 'tblzones.id')
            ->whereBetween('hms_camp_leads.created_at', [$startDateFormatted, $endDateFormatted]);
        ...
        // access_limit == 1 => Super Admin – no restriction
        // dd($data->get());
        // 🧩 Dynamic filters from frontend - FIXED VERSION
        if ($fitterremovedataall) {
            foreach (explode(' AND ', $fitterremovedataall) as $condition) {
                if (strpos($condition, '=') !== false) {
                    [$column, $value] = explode('=', $condition);
                    ...
                    if ($column === 'tbl_locations.name') {
                        // For location name, we need to filter on the joined table
                        $data->whereIn('tbl_locations.name', explode(',', $value));
                    } elseif ($column === 'tblzones.name') {
                        // For zone name, we need to filter on the joined table
                        $data->whereIn('tblzones.name', explode(',', $value));
                    }
                }
            }
        }
        ...
        // Add debug info if results are empty
        if ($results->isEmpty()) {
            \Log::info('No results found for filters', [
                'filters' => $fitterremovedataall,
                'date_range' => $datefiltervalue,
                'sql' => $data->toSql(),
...
//     // Get parent menus the user has access to
//     $userMenus = DB::table('user_menus as um')
//         ->join('menus as m', 'm.id', '=', 'um.menu_id')
...
//     $menusWithChildren = $userMenus->map(function ($menu) {
//         // Get children where sub_menus == current menu id
//         $children = DB::table('menus')
//             ->where('sub_menus', $menu->id)
//             ->select('id', 'menu_name', 'route', 'icon', 'sub_menus', 'dropdown', 'active_ids')
//             ->get();

//         // Add children to current menu
//         $menu->children = $children;
      ...
      public function allticketDateFillter(Request $request)
    {
        ...
        $data = TicketDetails::select(
                        ...
                        if($role_id == 1 && $access_limits == 2){
							...
							$data->join('tbl_department_user', 'tblzones.id', '=', 'tbl_department_user.zone_id');
    ...
    public function dailyDateFilter(Request $request)
    {
        ...
        if ($request->apistatus == 'checkinreport') {
            ...
            $areaMapping = [
                ...
                'AFMDU' => 'Madurai',
                'AFKAN' => 'Bangalore',
                'AFECT' => 'Bengaluru - Electronic City',
                'Aathur' => 'Aathur',
...
//     // Validate request
//     // $validatedData = $request->validate([
//     //     'dis_sno' => 'required|string|max:255',
//     //     'dis_zone_id' => 'required|string|max:255',
//     //     'dis_s_no' => 'required|string|max:255',
...
//     // extra fields
//     // $model->dis_sno   = $request->dis_sno ?? null;
//     // $model->dis_s_no  = $request->dis_s_no ?? null;
...
//         'dis_expected_request'=>'nullable|string|max:255',
//         'dis_post_discount' => 'nullable|string|max:255',
//         'dis_counselled_by' => 'nullable|string|max:255',
//         'dis_final_auth' => 'nullable|string|max:255',
//         'dis_branch_no' => 'nullable|string|max:255',
//         'dis_auth_by' => 'nullable|string|max:255',
//         'dis_approved_by' => 'nullable|string|max:255',
...
//             $imagePaths[$field] = json_encode(['discount_form/' . $filename]);
//         } else {
//             $existing = DiscountFormModel::where('dis_wife_mrd_no', $wifemrdno)->where('dis_husband_mrd_no', $husmrdno)
//             ->value($field);
...
// // } else {
// //     $dataToSave['dis_wife_mrd_no'] = $wifemrdno;
// //     $dataToSave['dis_husband_mrd_no'] = $husmrdno;
// //      $dataToSave['created_by'] = auth()->id();
// //     DiscountFormModel::create($dataToSave);
// // }
// if ($record) {
//     $record->fill($dataToSave);
...
//     $dataToSave['dis_wife_mrd_no'] = $wifemrdno;
//     $dataToSave['dis_husband_mrd_no'] = $husmrdno;
//     $record = new DiscountFormModel($dataToSave);
//     $record->created_by = auth()->id();
// }
...
// return response()->json(['success' => true, 'message' => 'Discount form saved successfully!']);
// }
// public function discountformeditsave(Request $request)
// {
//     $admin = auth()->user();
...
//     $model = DiscountFormModel::firstOrNew([
//         'dis_husband_mrd_no' => $validated['dis_husband_mrd_no'],
//     ]);
...
//             $path = 'discount_form/' . $filename;

//             $image->move(public_path('discount_form'), $filename);

//             $model->$field = json_encode([$path]);
//         }
//         // else → keep existing value automatically
//     }

//     // Force fields (like your Cancelbill code)
//     $model->created_by = $admin->id;
//     $model->status = 1;
...
// {
//     $admin = auth()->user();
//     return view('superadmin.masteraccess', ['admin' => $admin]);
// }
...
//     // Get state/zone information
//     preg_match("/'([^']+)'/", $fitterremovedataall, $matches);
//     $state = $matches[1] ?? '';

//     // Check if filtering by zone or specific branch
...
//         if ($isZoneFilter) {
//             $zone_locations = $this->zoneQuery($state);
//             $allBranchesWithZero = $this->prepareZeroDataForAllBranches($zone_locations);
//             return response()->json(['data' => $allBranchesWithZero, 'dropdown' => $zone_locations]);
//         } else {
//             $zone_locations = TblLocationModel::orderBy('name', 'asc')->get();
//             return response()->json(['data' => $finalOutput, 'dropdown' => $zone_locations]);
//         }
...
//     if ($isZoneFilter && $isBranchFilter) {
//         // Extract specific branch name
//         preg_match("/tbl_locations\.name\s*=\s*'([^']*)'/", $fitterremovedataall, $branchMatches);
//         $selectedBranch = $branchMatches[1] ?? '';

//         // Get all branches in the zone for dropdown
//         $zone_locations = $this->zoneQuery($state);

//         // Filter to show only the selected branch data
//         $filteredOutput = [];
...
//                 // Calculate totals
//                 $totalCash += $item['cash_amt'] ?? 0;
//                 $totalUpi += $item['upi_amt'] ?? 0;
//                 $totalCard += $item['card_amt'] ?? 0;
//                 $totalNeft += $item['neft_amt'] ?? 0;
//                 $totalTotal += $item['total_amt'] ?? 0;
//             }
//         }

//         // If no data for selected branch, create zero data
//         if (empty($filteredOutput)) {
//             $selectedBranchData = TblLocationModel::where('name', $selectedBranch)->first();
//             if ($selectedBranchData) {
//                 $zeroData = [
//                     'type' => 'Branch',
//                     'billdate' => '',
//                     'area' => $selectedBranch,
//                     'zone_name' => $selectedBranchData->zone_name ?? $state,
//                     'cash_amt' => '0.00',
//                     'upi_amt' => '0.00',
//                     'card_amt' => '0.00',
//                     'neft_amt' => '0.00',
...
//         $newArray = [
//             "type" => "Consolidated",
//             "billdate" => $filteredOutput[0]['billdate'] ?? '',
//             "area" => 'All',
//             "zone_name" => $filteredOutput[0]['zone_name'] ?? $state,
//             "total_cash_amt" => number_format($totalCash, 2, '.', ''),
//             "total_upi_amt" => number_format($totalUpi, 2, '.', ''),
//             "total_card_amt" => number_format($totalCard, 2, '.', ''),
//             "total_neft_amt" => number_format($totalNeft, 2, '.', ''),
//             "total_total_amt" => number_format($totalTotal, 2, '.', ''),
//             "zone_type" => 2,
//         ];
//         $filteredOutput[] = $newArray;

//         return response()->json(['data' => $filteredOutput, 'dropdown' => $zone_locations]);
//     }
...
//         // Create map of branches with data
//         $branchesWithData = [];
//         foreach ($finalOutput as $item) {
//             $branchName = $item['area'] ?? $item['zone_name'] ?? '';
//             if ($branchName) {
//                 $branchesWithData[$branchName] = $item;
//             }
//         }

//         // Merge with all branches in zone
//         $mergedOutput = [];
//         foreach ($zone_locations as $branch) {
//             $branchName = $branch->name;

//             if (isset($branchesWithData[$branchName])) {
//                 $branchData = $branchesWithData[$branchName];
//                 $branchData['zone_type'] = 2;
//                 $mergedOutput[] = $branchData;

//                 // Add to totals
//                 $totalCash += $branchData['cash_amt'] ?? 0;
//                 $totalUpi += $branchData['upi_amt'] ?? 0;
//                 $totalCard += $branchData['card_amt'] ?? 0;
//                 $totalNeft += $branchData['neft_amt'] ?? 0;
//                 $totalTotal += $branchData['total_amt'] ?? 0;
//             } else {
//                 // Create zero data for missing branch
//                 $zeroBranchData = [
//                     'type' => 'Branch',
//                     'billdate' => '',
//                     'area' => $branchName,
//                     'zone_name' => $branch->zone_name ?? $state,
//                     'cash_amt' => '0.00',
//                     'upi_amt' => '0.00',
//                     'card_amt' => '0.00',
//                     'neft_amt' => '0.00',
...
//         $newArray = [
//             "type" => "Consolidated",
//             "billdate" => $mergedOutput[0]['billdate'] ?? '',
//             "area" => 'All',
//             "zone_name" => $state,
//             "total_cash_amt" => number_format($totalCash, 2, '.', ''),
//             "total_upi_amt" => number_format($totalUpi, 2, '.', ''),
...
//             "zone_type" => 2,
//         ];
//         $mergedOutput[] = $newArray;

//         return response()->json(['data' => $mergedOutput, 'dropdown' => $zone_locations]);
//     }
...
//         // Check if it's a zone-based branch or standalone branch
//         if (strpos($fitterremovedataall, "tblzones.name=") !== false) {
//             $zone_locaton = TblLocationModel::select('zone_id')->where('name', $ilocation)->first();
//             $zone_location = TblLocationModel::where('zone_id', $zone_locaton->zone_id ?? null)->get();
//         } else {
//             $zone_location = TblLocationModel::orderBy('name', 'asc')->get();
//         }

//         // Filter to show only the selected branch
//         $filteredOutput = [];
//         foreach ($finalOutput as $item) {
//             $branchName = $item['area'] ?? $item['zone_name'] ?? '';
//             if ($branchName === $ilocation) {
//                 $item['zone_type'] = 2;
//                 $filteredOutput[] = $item;
...
//         // Add zone_type to all items
//         foreach ($filteredOutput as $key => $item) {
//             $filteredOutput[$key]['zone_type'] = 2;
//         }

//         // Add consolidated row if we have data
//         if (!empty($filteredOutput)) {
//             $newArray = [
//                 "type" => "Consolidated",
//                 "billdate" => $filteredOutput[0]['billdate'] ?? '',
//                 "area" => 'All',
//                 "zone_name" => $filteredOutput[0]['zone_name'] ?? '',
//                 "total_cash_amt" => number_format($totalCash, 2, '.', ''),
//                 "total_upi_amt" => number_format($totalUpi, 2, '.', ''),
//                 "total_card_amt" => number_format($totalCard, 2, '.', ''),
//                 "total_neft_amt" => number_format($totalNeft, 2, '.', ''),
...
//         return response()->json(['data' => $filteredOutput, 'dropdown' => $zone_location]);
//     }
// }
private function incomeBranchDateFiltr($fitterremovedataall, $moredatefittervale, $apistatus, $statusid)
{
...
private function zoneQuery($city)
   ...
   public function getart(){
    ...
    public function baseline(){
       $staffId = auth()->user()->id;
        $admin = auth()->user();
        return view('superadmin.baseline', compact('admin'));
    }

     public function ICSI(){
       $staffId = auth()->user()->id;
        $admin = auth()->user();
        return view('superadmin.ICSI', compact('admin'));
    }
    ...
    public function ovarian_stimulation(){
     ...
     public function opu(){
        $staffId = auth()->user()->id;
        $admin = auth()->user();
        return view('superadmin.opu', compact('admin'));

    }

    public function registerivf(){
        $staffId = auth()->user()->id;
        $admin = auth()->user();
        return view('superadmin.registerivf', compact('admin'));

    }

    public function Registerviewpage(){
        $staffId = auth()->user()->id;
        $admin = auth()->user();
        return view('superadmin.Registerviewpage', compact('admin'));

    }

    public function checkinviewpages(){
        $staffId = auth()->user()->id;
        $admin = auth()->user();
        return view('superadmin.checkinviewpages', compact('admin'));

    }

     public function OvarianStimulationivf(){
        $staffId = auth()->user()->id;
        $admin = auth()->user();
        return view('superadmin.OvarianStimulationivf', compact('admin'));
    ...
    public function culture(){
        $staffId = auth()->user()->id;
        $admin = auth()->user();
        return view('superadmin.culture', compact('admin'));

    }
     public function EmbryoTransfer(){
        $staffId = auth()->user()->id;
        $admin = auth()->user();
        return view('superadmin.EmbryoTransfer', compact('admin'));

    }

    public function BetaHCg(){
        $staffId = auth()->user()->id;
        $admin = auth()->user();
        return view('superadmin.BetaHCg', compact('admin'));
...
//         if (!empty($admin->zone_id)) {
//             $data->where('tblzones.id', $admin->zone_id);
...
//         \Log::info('User access - Branch ID: ' . $admin->branch_id . ', Created By: ' . $admin->id);

//     } else {
//         // ❌ Unknown access level - deny access
//         \Log::warning('Unknown access level: ' . $admin->access_limits);
//         return response()->json([]);
//     }
...
//     ->join('tblzones', 'tbl_locations.zone_id', '=', 'tblzones.id')
//     ->whereDate('hms_cancelbill_form.created_at', '>=', $startdates)->where('hms_cancelbill_form.created_at', '<=', $enddates);
...
//             }
//         }
//         $data = $data->orderBy('hms_cancelbill_form.created_at', 'desc')->get();
...
//     $checkinData = $this->getCheckinData($locations, $hasZone, $hasLocation, $hasPhid, $values, $start_date, $end_date, $mrdno);
...
// public function refundformeditsave(Request $request){
//     $wifemrdno =$request->input('ref_wife_mrd_no');
//     $husmrdno = $request->input('ref_husband_mrd_no');
//     $validatedData = $request->validate([
//         'ref_zone_id' => 'required|string|max:255',
//         'ref_wife_mrd_no'=> 'nullable|string|max:255',
//         'ref_husband_mrd_no'=> 'nullable|string|max:255',
...
//     ]);
//     $imagePaths = [];
//     $signatureFields = ['ref_wife_sign', 'ref_husband_sign', 'ref_drsign', 'ref_cc_sign', 'ref_admin_sign'];
...
//             $imagePaths[$field] = json_encode(['refund_form/' . $filename]);
//         } else {
//             $existing = RefundFormModel::where('ref_wife_mrd_no', $wifemrdno)->where('ref_husband_mrd_no', $husmrdno)
//             ->value($field);
//             $imagePaths[$field] = $existing ?: json_encode([]);
//         }
//     }
//  $record = RefundFormModel::where('ref_wife_mrd_no', $wifemrdno)
//     ->where('ref_husband_mrd_no', $husmrdno)
//     ->first();

// $dataToSave = array_merge($validatedData, [
//     'ref_wife_sign' => $imagePaths['ref_wife_sign'],
//     'ref_husband_sign' => $imagePaths['ref_husband_sign'],
//     'ref_drsign' => $imagePaths['ref_drsign'],
//     'ref_cc_sign' => $imagePaths['ref_cc_sign'],
//     'ref_admin_sign' => $imagePaths['ref_admin_sign'],
//     'status' => 1,
// ]);
...
//     $record->update($dataToSave);
// } else {
//     $dataToSave['ref_wife_mrd_no'] = $wifemrdno;
//     $dataToSave['ref_husband_mrd_no'] = $husmrdno;
//     RefundFormModel::create($dataToSave);
// }
// return response()->json(['success' => true, 'message' => 'Refund form saved successfully!']);
// }
...
//     $admin = auth()->user();
//     return view('superadmin.cancelbillform',['admin'=>$admin]);
// }
public function cancelbillform(){
    $admin = auth()->user();
    $approver = DB::table('cancel_bill_approver')
