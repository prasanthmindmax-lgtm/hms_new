...
class QuotationImport implements ToCollection, WithHeadingRow
{
   public function collection(Collection $rows)
    {
        $grouped = $rows->groupBy('quotation_no');
        $now = now();
        $user_id = auth()->user()->id;

        foreach ($grouped as $quotation_no => $lines) {
            $first = $lines->first();

            // ✅ Add validation for required fields
            if (!isset($first['vendor_id'])) {
                Log::error("Missing vendor_id for quotation: " . $quotation_no);
            ...
            if (empty($vendor_id)) {
                Log::error("Empty vendor_id for quotation: " . $quotation_no);
                continue;
            }
            ...
            $count = TblQuotation::count();
            $nextNumber = $count + 1;
            $quotation_gen_no = 'QO-' . str_pad($nextNumber, 3, '0', STR_PAD_LEFT);
            ...
            $quotation = TblQuotation::create([
                ...
                'quotation_gen_no' => $quotation_gen_no,
