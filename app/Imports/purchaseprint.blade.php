...
while ($currentItem < $itemCount) {
    ...
    // Check if we need a new page BEFORE drawing the row (leave room for summary)
    if ($y + $maxHeight > 190) {
        $pdf->AddPage();
...
// ================== PAGE MANAGEMENT FOR SUMMARY ==================

// Always add a new page for summary if we had multiple pages of items
if (!$isFirstPage) {
    $pdf->AddPage();
    $summaryY = 30; // Start summary at top of new page
} else {
    // On first page, start summary after items
    $summaryY = max($y, 140) + 10; // Ensure minimum starting position
}
...
if($purchase->export_name === null){
    ...
    $pdf->Cell(60, 8, 'Balance Due : ₹' . number_format($purchase->balance_amount ?? 50000.00, 2), 0, 1, 'C', true);
    $balanceDueY = $summaryY + 42;
}

// ================== NOTE SECTION ==================
$noteText = $purchase->note ?? '';
$noteY = $balanceDueY + 15;

if (!empty($noteText)) {
    // Check if we have enough space for note
    $currentPageHeight = $pdf->GetPageHeight();
    if ($noteY + 40 > $currentPageHeight) {
        $pdf->AddPage();
        $noteY = 30;
    }
    ...
    // Get the Y position after the note
    $signatureY = $pdf->GetY() + 10;
} else {
    // If no note, position signature after balance due
    $signatureY = $balanceDueY + 25;
}

// ================== AUTHORIZED SIGNATURE ==================
// Check if signature fits on current page
$currentPageHeight = $pdf->GetPageHeight();
if ($signatureY + 20 > $currentPageHeight) {
    $pdf->AddPage();
    $signatureY = 30;
}
